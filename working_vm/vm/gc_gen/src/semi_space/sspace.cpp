...
#ifdef ORDER
void nos_swap_obj_header_info(Sspace* sspace)
{
...
#ifdef ORDER_DEBUG
        assert(!obj_belongs_to_tospace((Partial_Reveal_Object *)block));
#endif
        swap_obj_header_info_in_block(block, MARK_IN_OI);
    }
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of nongen semispace pool algo ...\n");
#endif
#else
    assert(0);
