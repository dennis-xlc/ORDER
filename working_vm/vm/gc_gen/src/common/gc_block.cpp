...
//1Note that this function should be bypassed definitly when release. It will cost much!!!!
void verify_header(Partial_Reveal_Object* obj)
{
#ifdef ORDER_DEBUG
    assert(obj);
    ...
    assert(gcvt);
    Class_Handle ch = gcvt->gc_clss;
    assert(ch);

    assert(gcvt->gc_allocated_size);
    assert(vm_object_size(obj) != 0);
	
    if(order_record){	
        assert(obj->alloc_tid);
        assert(obj->access_tid);
        assert(obj->alloc_count);
        assert(obj->access_count);
    }
    else if(replay_verify){
        assert(obj->alloc_tid);
        assert(obj->access_tid);
...
void swap_obj_header_info_in_block(Block_Header* block, int mark_type)
{
   
    if(block->status !=  BLOCK_FREE){
#ifdef ORDER_DEBUG
        assert(block->status == BLOCK_USED);
#endif
        Partial_Reveal_Object *cur_obj = (Partial_Reveal_Object *)block->base;
        Partial_Reveal_Object *block_end = (Partial_Reveal_Object *)block->free;
		
#ifdef ORDER_GC_DEBUG
        Partial_Reveal_Object *last_obj = NULL;
#endif
        ...
        while(cur_obj < block_end){
            ...
            verify_header(cur_obj);
#endif

            if(mark_type == MARK_IN_OI){
                if(obj_is_fw_in_oi(cur_obj)){
                }
	       else{
		  if(order_record){
                        record_access_info(cur_obj);
		  }
#ifdef ORDER_DEBUG//yzm
		  else if(replay_verify){
		      if(cur_obj->alloc_count == -1){
//                            assert(cur_obj->access_count == -1);
                    }
                    else{
                        assert(cur_obj->access_count == 0);
                    }
                 }
#endif
	       }
...
//                            assert(cur_obj->access_count == -1);
            ...
            else if(mark_type == MARK_IN_VT_FORWARD){
	       if(obj_is_marked_in_vt(cur_obj) || obj_is_fw_in_oi(cur_obj)){
                }
	       else{
                    if(order_record){
...
#ifdef ORDER_GC_DEBUG
            Partial_Reveal_Object *last_obj = cur_obj;
#endif

#ifdef USE_32BITS_HASHCODE
            /* In minor gc (semi-space), if an object is forwarded, 
                 its orignal hashcode bit is used to store forwarded pointer,
                 but its hashcode bit then is set in oi by obj_sethash_in_vt, 
                 and we use obj_set_size_extend() to indicate the object is forwarded,
                 so we first check whether this object is forwarded by  obj_size_extend(),
                 if so we use obj_end_extend() to get next obj, otherwise use obj_end().
            */
            if(obj_size_extend(cur_obj)){//the object is forwarded in semi-space, its size should use obj_end_extend
                reset_obj_size_extend(cur_obj);
                cur_obj = obj_end_extend(cur_obj);
            }
            else{
