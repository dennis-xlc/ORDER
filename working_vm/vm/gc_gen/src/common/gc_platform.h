...
#ifdef ORDER
    ((hythread_t)ret_thread)->p_tid = hythread_self()->thread_id;
    hythread_self()->thread_create_count++;
    ((hythread_t)ret_thread)->p_count = hythread_self()->thread_create_count;
#endif
...
#define seq_write_lock(lock,v) while(true){ v=lock;if((v&0x1) == 0){if(atomic_cas32(&lock, v+1, v)){break;}} vm_thread_yield();}
#define seq_write_unlock(lock) lock++;
#define seq_read_lock(lock,v) while(true){ v=lock; if ((v&0x1) != 0) continue;
#define seq_read_unlock(lock,v) if (v == lock) break; vm_thread_yield();}
