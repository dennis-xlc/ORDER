...
#ifdef ORDER
  if (vm_property_is_set("order.record_mode", VM_PROPERTIES) == 1) {
    order_record = TRUE;
  }
  else if(vm_property_is_set("order.replay_mode", VM_PROPERTIES) == 1){
    order_record = FALSE;
  }
  
  if (vm_property_is_set("gc.record_in_binary", VM_PROPERTIES) == 1) {
    binary_mode = vm_property_get_boolean("gc.record_in_binary");
  }
#endif
