...
void gc_gen_collect_access_info_heap(GC_Gen *gc)
{
	   ...
	   if(order_record){
                record_access_info((Partial_Reveal_Object* )p_obj);
	       		...
	       		if(((Partial_Reveal_Object* )p_obj)->alloc_count == -1){
                     assert(((Partial_Reveal_Object* )p_obj)->access_count <= -1);
                }
                else{
                      while(((Partial_Reveal_Object* )p_obj)->access_count != 0)
                      {
                          printf("object not dead.\n");
                          usleep(100);
                      }
                 }
             }
...
//  space_end = (Block_Header*)&fspace->blocks[fspace->free_block_idx - fspace->first_block_idx];
        ...
        if (!obj_vt_is_to_next_obj((Partial_Reveal_Object *)p_obj)){
	       ...
	       if(((Partial_Reveal_Object* )p_obj)->alloc_count == -1){
                     //assert(((Partial_Reveal_Object* )p_obj)->access_count == -1);
                      ...
                      assert(((Partial_Reveal_Object* )p_obj)->access_count == 0);
              }
	     }
#endif
        }
  ...
  while (lspace_obj < lspace_end) {
    if(!*((unsigned int *)lspace_obj)){
      ...
      if(order_record){
          record_access_info((Partial_Reveal_Object* )lspace_obj);
      }
#ifdef ORDER_DEBUG
      else if(replay_verify){
          if(((Partial_Reveal_Object* )lspace_obj)->alloc_count == -1){
               //assert(((Partial_Reveal_Object* )lspace_obj)->access_count == -1);
          }
          else{
                assert(((Partial_Reveal_Object* )lspace_obj)->access_count == 0);
          }
       }
#endif
#ifdef USE_32BITS_HASHCODE
...
#ifdef ORDER_VERIFY
void gc_gen_scan_heap(GC_Gen * gc)
{
  Mutator *mutator = gc->mutator_list;
  bool cont = true;   
  while (mutator) {
  ...
  Block_Header *curr_block = (Block_Header*)mspace->blocks;
  U_32 current_idx = mspace->first_block_idx;
  U_32 ceiling_idx = (unsigned int)( ((U_32)mspace->heap_end - (U_32)mspace->heap_start) >> GC_BLOCK_SHIFT_COUNT);
    ...
    if(curr_block->status !=  BLOCK_FREE){
        ...
        while(p_obj < block_end){
          if (obj_vt_is_to_next_obj((Partial_Reveal_Object *)p_obj)) 
            ...
            if (((Partial_Reveal_Object*)p_obj)->alloc_tid == 2 &&
                ((Partial_Reveal_Object*)p_obj)->alloc_count == 50179)
                printf("here we got it in mos!!!\n");
...
//        assert(0);
  ...
  curr_block = (Block_Header*)fspace->blocks;
  current_idx = fspace->first_block_idx;
  ceiling_idx = (unsigned int)( ((U_32)fspace->heap_end - (U_32)fspace->heap_start) >> GC_BLOCK_SHIFT_COUNT);
//  space_end = (Block_Header*)&fspace->blocks[fspace->free_block_idx - fspace->first_block_idx];
  space_size = (Block_Header*)&fspace->blocks[ceiling_idx];
  while(curr_block < space_size) {
    if(curr_block->status !=  BLOCK_FREE){
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: start minor collection ...\n");
#endif
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of minor collection ...\n");
#endif
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: start major collection ...\n");
#endif
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of major collection ...\n");
#endif 
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: Minor collection failed, transform to fallback collection ...\n");
#endif 
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of fallback collection ...\n");
#endif 
  }
