...
#ifdef ORDER
void lspace_swap_obj_header_info(Lspace* lspace, int mark_type)
{
#ifdef ORDER_GC_DEBUG
    if(mark_type == MARK_IN_OI){
        printf("[GC DEBUG]: lspace swap obj header info ,mark type is MARK_IN_OI\n");
    }
    else if(mark_type == MARK_IN_VT){
        printf("[GC DEBUG]: lspace swap obj header info ,mark type is MARK_IN_VT\n");
    }
#endif	
    ...
    //statistics info
//    unsigned int live_num = 0, dead_num = 0;
    ...
    while (lspace_obj < lspace_end) {
        if(!*((unsigned int *)lspace_obj)){
            ...
            p_obj = (Partial_Reveal_Object *)lspace_obj;	   

            if(mark_type == MARK_IN_OI){
                if(obj_is_marked_in_oi(p_obj)){
//                    live_num++;
                }
                else{
//                    dead_num++;
                    if(order_record){
                        record_access_info(p_obj);
                    }
#ifdef ORDER_DEBUG
		      else if(replay_verify){
		          if(p_obj->alloc_count == -1){
                            assert(p_obj->access_count <= -1);
                        }
                        else{
...
//                    dead_num++;
            ...
            hash_extend_size  = (hashcode_is_attached(p_obj))?GC_OBJECT_ALIGNMENT:0;
#endif
            unsigned int obj_size = (unsigned int)ALIGN_UP_TO_KILO(vm_object_size(p_obj)+hash_extend_size);
            lspace_obj = lspace_obj + obj_size;
        }
    }

//    printf("[xlc]: The total object in lspace is %d, live num is %d, dead num is %d\n", live_num+dead_num, live_num, dead_num);
...
{
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: start lspace_sliding_compact ... \n");
#endif    

#ifdef ORDER
  if(order_record){
      lspace_swap_obj_header_info(lspace, MARK_IN_VT);
  }
#endif
#ifdef ORDER_DEBUG
  if(replay_verify){
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of lspace_sliding_compact ... \n");
#endif 
  return;
}
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: lspace sweep algo start ...\n");
#endif

#ifdef ORDER
  if(order_record){
      lspace_swap_obj_header_info(lspace, MARK_IN_OI);
  }
#endif
#ifdef ORDER_DEBUG
  if(replay_verify){
...
#ifdef ORDER_GC_DEBUG
    printf("[GC DEBUG]: end of lspace sweep algo ...\n");
#endif
  return;
}
