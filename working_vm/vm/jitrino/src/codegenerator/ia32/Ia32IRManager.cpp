...
bool IRManager::isOrderGCSafePoint(const Inst* inst) {
    if (inst->getMnemonic() == Mnemonic_CALL) {
        const CallInst* callInst =  (const CallInst*)inst;
        Opnd* target = callInst->getOpnd(callInst->getTargetOpndIndex());
        if (target->isPlacedIn(OpndKind_Imm))
        {
            if (target->getImmValue() == (long)((POINTER_SIZE_INT)ORDERRep::try_lock_obj) ||
			target->getImmValue() == (long)((POINTER_SIZE_INT)ORDERRep::usleep_debug) ||
			target->getImmValue() == (long)((POINTER_SIZE_INT)ORDERRep::usleep_try_lock_obj_dummy))
            {
                return true;
            }
        }
