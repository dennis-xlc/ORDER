        ...
        for (Nodes::const_reverse_iterator it = postOrdered.rbegin(), end = postOrdered.rend(); it!=end; ++it) {
            ...
            for (Inst * inst = (Inst*)node->getFirstInst(); inst != NULL; inst=inst->getNextInst()){
                ...
                if (inst->getParentObjectLoad()){
                    Opnd * opnd=inst->getParentObjectLoad();
                    U_32 opndId = opnd->getId();
#ifdef ORDER_DEBUG
assert(irManager->getOpndCount() >opndId );
#endif
                    ...
                    if (opndInfo.sourceOpndId != EmptyUint32){
                        ...
                        //2Here the comparison is deleted in ORDER
                        opndInfo.sourceOpndId = EmptyUint32;
                ...
                if (inst->getParentObjectStore()){
                    Opnd * opnd=inst->getParentObjectStore();
                    U_32 opndId = opnd->getId();
#ifdef ORDER_DEBUG
assert(irManager->getOpndCount() >opndId );
#endif
...
#ifdef ORDER
            Inst ** originalInsts = new(mm) Inst* [opndCount];
#endif

            memset(replacements, 0, sizeof(Opnd*) * opndCount);
#ifdef ORDER  //yzm
            memset(originalInsts, 0, sizeof(Inst*) * opndCount);
#endif
...
#ifdef ORDER
                        inst->replaceOpnds(replacements, OpndRole_All, originalInsts);
#else
