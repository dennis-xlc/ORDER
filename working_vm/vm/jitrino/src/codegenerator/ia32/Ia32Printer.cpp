...
void IRPrinter::printInst(const Inst * inst)
{
...
#ifdef ORDER //yzm
    {
        Opnd* obj = inst->getParentObjectLoad();
        if(obj)
        {
            os << "[load obj: ";
            printOpnd(obj, Inst::OpndRole_Null, false, false);
            os << "]";
        }
    }
    {
        Opnd* obj = inst->getParentObjectStore();
        if(obj)
        {
            os << "[store obj: ";
            printOpnd(obj, Inst::OpndRole_Null, false, false);
    ...
    {
        Class_Handle clz = inst->getParentClassLoad();
        if(clz)
        {
            os << "[load clz: " << (long)clz << "]";
        }

    }
    {
        Class_Handle clz = inst->getParentClassStore();
        if(clz)
        {
            os << "[store clz: " << (long)clz << "]";
        }
    }
#endif
