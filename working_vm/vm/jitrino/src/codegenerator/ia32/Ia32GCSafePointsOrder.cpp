...
#include "Ia32Inst.h"
#include "Ia32GCSafePointsOrder.h"
#include "XTimer.h"
#include "BitSet.h"
...
static ActionFactory<GCPointsBaseLiveRangeFixerOrder> _gcpoints ("order_gcpoints");
...
void OrderGCSafePointsInfo::removePairByMPtrOpnd(GCSafePointPairs& pairs, const Opnd* mptr) const {
#ifdef _DEBUG_
    U_32 nPairs = 0;
#endif 
...
MPtrPair* OrderGCSafePointsInfo::findPairByMPtrOpnd(GCSafePointPairs& pairs, const Opnd* mptr) {
...
OrderGCSafePointsInfo::OrderGCSafePointsInfo(MemoryManager& _mm, IRManager& _irm, Mode _mode) :
...
bool OrderGCSafePointsInfo::graphHasSafePoints(const IRManager& irm){
...
bool OrderGCSafePointsInfo::blockHasSafePoints(const Node* b) {
    for (Inst * inst=(Inst*)b->getFirstInst(); inst!=NULL; inst=inst->getNextInst()){
        if (isOrderGCSafePoint(inst)) {
            return true;
        }
    }
    return false;
...
U_32 OrderGCSafePointsInfo::getNumSafePointsInBlock(const Node* b, const GCSafePointPairsMap* pairsMap ) {
    U_32 n = 0;
    for (Inst * inst=(Inst*)b->getFirstInst(); inst!=NULL; inst=inst->getNextInst()){
        if (isOrderGCSafePoint(inst)) {
...
void OrderGCSafePointsInfo::_calculate() {
...
void OrderGCSafePointsInfo::insertLivenessFilters() {
...
void OrderGCSafePointsInfo::setLivenessFilter(const Inst* inst, const BitSet* ls) {
    livenessFilter[inst] = ls;
}

const BitSet* OrderGCSafePointsInfo::findLivenessFilter(const Inst* inst) const {
...
void OrderGCSafePointsInfo::calculateMPtrs() {
        ...
        for (Nodes::const_reverse_iterator it = postOrderedNodes.rbegin(), end = postOrderedNodes.rend(); it!=end; ++it) {
            ...
            if (node->isBlockNode()) {
                for (Inst * inst=(Inst*)node->getFirstInst(); inst!=NULL; inst=inst->getNextInst()){
                    ...
                    if (isOrderGCSafePoint(inst)) { //saving results on safepoint
...
I_32 OrderGCSafePointsInfo::getOffsetFromImmediate(Opnd* offsetOpnd) const {
...
void OrderGCSafePointsInfo::runLivenessFilter(Inst* inst, GCSafePointPairs& pairs) const {
    if (!pairs.empty()) {
...
Opnd* OrderGCSafePointsInfo::getBaseAccordingMode(Opnd* opnd) const {
...
void OrderGCSafePointsInfo::updateMptrInfoInPairs(GCSafePointPairs& res, Opnd* newMptr, Opnd* fromOpnd, int offset, bool fromOpndIsBase) {
...
void OrderGCSafePointsInfo::filterStaticMptrs(Inst* inst) {
...
void OrderGCSafePointsInfo::updatePairsOnInst(Inst* inst, GCSafePointPairs& res) {
    ...
    //yzm, order added, LEA is generated after original action : gcpoints
    if (inst->getMnemonic() == Mnemonic_LEA && inst->getOpnd(defIndex)->getType()->isObject() == false && 
		inst->getOpnd(useIndex1)->getType()->isObject() == false)
        return;
...
void OrderGCSafePointsInfo::derivePairsOnEntry(const Node* node, GCSafePointPairs& res) {
    assert(res.empty());
...
void OrderGCSafePointsInfo::filterLiveMPtrsOnGCSafePoints() {
...
#ifdef _DEBUG
#if defined(ORDER_DEBUG) && !defined(RECORD_STATIC)
...
void OrderGCSafePointsInfo::checkPairsOnNodeExits() const {
...
void OrderGCSafePointsInfo::checkPairsOnGCSafePoints() const {
...
void OrderGCSafePointsInfo::dump(const char* stage) const {
    Log::out()<<"========================================================================"<<std::endl;
...
void GCPointsBaseLiveRangeFixerOrder::runImpl() {
    bool disableStaticOffsets = false;
    getArg("disable_static_offsets", disableStaticOffsets);
    MemoryManager mm("OrderGCSafePointsMarker");
    OrderGCSafePointsInfo info(mm, *irManager, OrderGCSafePointsInfo::MODE_1_FIX_BASES);
    ...
    for (Nodes::const_iterator it = nodes.begin(), end = nodes.end(); it!=end; ++it) {
        ...
        for (Inst * inst=(Inst*)node->getFirstInst(); inst!=NULL; inst=inst->getNextInst()) {
            if (IRManager::isOrderGCSafePoint(inst)) {
