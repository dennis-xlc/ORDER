...
#ifdef ORDER
//_________________________________________________________________________________________________
bool Inst::replaceOpnd(Opnd * oldOpnd, Opnd * newOpnd, U_32 opndRoleMask, Inst* inst)
{
    bool replaced = false;
    assert(newOpnd != NULL);
    for (U_32 i=0, n=getOpndCount(); i<n; i++){
        ...
        if (
            ...
            if (opnd==oldOpnd){
                ...
                //yzm
                //ORDER: pass the accessed object information
                if (inst)
                {
                    if (inst->getParentObjectLoad())
                    {
                        setParentObjectLoad(inst->getParentObjectLoad());
                    }
                    if (inst->getParentObjectStore())
                    {
                        setParentObjectStore(inst->getParentObjectStore());
                    }
                    if (inst->getParentClassLoad())
                    {
                        setParentClassLoad(inst->getParentClassLoad());
                    }
                    if (inst->getParentClassStore())
                    {
                        setParentClassStore(inst->getParentClassStore());
                    }
                }
                opnd=newOpnd;
                replaced = true;
...
#ifdef ORDER
//_________________________________________________________________________________________________
bool Inst::replaceOpnds(Opnd * const * opndMap, U_32 opndRoleMask, Inst * const * instMap)
{
    bool replaced = false;
    for (U_32 i=0, n=getOpndCount(); i<n; i++){
        U_32 opndRole=getOpndRoles(i);
        if (
            ...
            if (newOpnd!=NULL){
                ...
                setOpnd(i, newOpnd);
                //yzm
                //ORDER pass accessed object information
                if (instMap)
                {
                    Inst* inst = instMap[opnd->getId()];
                    if (inst && inst->getParentObjectLoad())
                    {
                        if (opndMap[inst->getParentObjectLoad()->getId()])
                        {
                            setParentObjectLoad(opndMap[inst->getParentObjectLoad()->getId()]);
                        }
                        else
                        {
                            setParentObjectLoad(inst->getParentObjectLoad());
                    ...
                    if (inst && inst->getParentObjectStore())
                    {
                        if (opndMap[inst->getParentObjectStore()->getId()])
                        {
                            setParentObjectStore(opndMap[inst->getParentObjectStore()->getId()]);
                        }
                        else
                        {
                            setParentObjectStore(inst->getParentObjectStore());
                    ...
                    if (inst && inst->getParentClassLoad())
                    {
                        setParentClassLoad(inst->getParentClassLoad());
                    }
                    if (inst && inst->getParentClassStore())
                    {
                        setParentClassStore(inst->getParentClassStore());
                    }
                }
