...
void InstCodeSelector::copyOpndTrivialOrTruncatingConversion(Opnd *dst, Opnd *src, Opnd* obj, Class_Handle clz, Field_Handle field, bool isLoad) 
{ 
    assert(dst->getSize()<=src->getSize());
#ifndef _EM64T_
    if (src->getType()->isInteger()&&src->getSize()==OpndSize_64)
    {
        Inst* inst = irManager.newI8PseudoInst(Mnemonic_MOV, 1, dst, src);
        if (isLoad && obj)
             inst->setParentObjectLoad(obj);
        else if (obj)
             inst->setParentObjectStore(obj);
        else if (isLoad)
             inst->setAccessFieldLoad(field);
        else
             inst->setAccessFieldStore(field);
/*
    //debugging instruction
    if (Log::isEnabled()){
    if (obj)
    {
        Log::out() << "[convert opnd-real: ";
        Opnd::DefScope ds=obj->getDefScope();
        Log::out()<<(
...
#endif
    {
        Inst* inst = irManager.newCopyPseudoInst(Mnemonic_MOV, dst, src);
        if (isLoad && obj)
             inst->setParentObjectLoad(obj);
        else if (obj)
             inst->setParentObjectStore(obj);
...
void InstCodeSelector::copyOpndTrivialOrTruncatingConversion(Opnd *dst, Opnd *src, Opnd* obj, Class_Handle clz, bool isLoad) 
{ 
    assert(dst->getSize()<=src->getSize());
#ifndef _EM64T_
    if (src->getType()->isInteger()&&src->getSize()==OpndSize_64)
    {
        ...
        if (isLoad && obj)
             ...
             inst->setParentClassLoad(clz);
        else
             inst->setParentClassStore(clz);
/*
    //debugging instruction
    if (Log::isEnabled()){
    if (obj)
...
#ifdef RECORD_STATIC_IN_FIELD
void InstCodeSelector::copyOpnd(Opnd *dst, Opnd *src, bool doZeroExtension, Opnd* obj, Class_Handle clz, Field_Handle field, bool isLoad) 
{ 
    convert(src, dst->getType(), dst, doZeroExtension, obj, clz, field, isLoad);
}
#endif

void InstCodeSelector::copyOpnd(Opnd *dst, Opnd *src, bool doZeroExtension, Opnd* obj, Class_Handle clz, bool isLoad) 
{ 
    convert(src, dst->getType(), dst, doZeroExtension, obj, clz, isLoad);
}
#endif
...
#ifdef ORDER
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertIntToInt(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj, Class_Handle clz, bool isLoad)
{
    ...
    if (dstSize==srcSize){ // trivial conversion
        if (dstOpnd==NULL && dstType == srcType) {
            ...
            copyOpndTrivialOrTruncatingConversion(dstOpnd, srcOpnd, obj, clz, isLoad);
        }
        ...
        if (dstOpnd==NULL)
            dstOpnd=irManager.newOpnd(dstType);
#ifdef _EM64T_
            Inst* inst = irManager.newInstEx(srcType->isSignedInteger() & !isZeroExtend ? Mnemonic_MOVSX:Mnemonic_MOVZX, 1, dstOpnd, srcOpnd);
            if (isLoad && obj)
                 inst->setParentObjectLoad(obj);
            else if (obj)
                 inst->setParentObjectStore(obj);
        ...
        if (dstSize<OpndSize_64){
            ...
            Inst* inst = irManager.newI8PseudoInst(srcType->isSignedInteger()?Mnemonic_MOVSX:Mnemonic_MOVZX, 1, dstOpnd, srcOpnd);
            if (isLoad && obj)
                 inst->setParentObjectLoad(obj);
            else if (obj)
                 inst->setParentObjectStore(obj);
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertIntToFp(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, Class_Handle clz, bool isLoad)
{
    ...
    if (srcSize<OpndSize_32) {
        srcOpnd=convert(srcOpnd, typeManager.getInt32Type(), NULL, false, obj, clz, isLoad);
    ...
    if (!useHelpers) { //use SSE
        if (srcSize == OpndSize_32) {
            ...
            if (isLoad && obj)
                 convInst->setParentObjectLoad(obj);
            else if (obj)
                 convInst->setParentObjectStore(obj);
            else if (isLoad)
                 convInst->setParentClassLoad(clz);
            else
                 convInst->setParentClassStore(clz);
            ...
            if (isLoad && obj)
                 copyInst->setParentObjectLoad(obj);
            else if (obj)
                 copyInst->setParentObjectStore(obj);
            else if (isLoad)
                 copyInst->setParentClassLoad(clz);
            else
                 copyInst->setParentClassStore(clz);
            ...
            if (isLoad && obj)
                 memAliasInst->setParentObjectLoad(obj);
            else if (obj)
                 memAliasInst->setParentObjectStore(obj);
            else if (isLoad)
                 memAliasInst->setParentClassLoad(clz);
            else
                 memAliasInst->setParentClassStore(clz);
            ...
            if (dstType->isDouble()) {
                ...
                if (isLoad && obj)
                     copy2SSEInst->setParentObjectLoad(obj);
                else if (obj)
                     copy2SSEInst->setParentObjectStore(obj);
                else if (isLoad)
                     copy2SSEInst->setParentClassLoad(clz);
                else
                     copy2SSEInst->setParentClassStore(clz);
                appendInsts(copy2SSEInst);
            } else {
                assert(dstSize == OpndSize_32);
                convertFpToFp(fpResOpnd, dstType, dstOpnd, obj, clz, isLoad);
            }
        }
    }
    ...
    if (useHelpers) {
        ...
        Opnd * args[] = {srcOpnd};
        Inst* inst = irManager.newInternalRuntimeHelperCallInst(helperName, 1, args, dstOpnd);
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertFpToInt(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, Class_Handle clz, bool isLoad)
{
    ...
    if (dstSize<OpndSize_32)
        dstOpnd=convert(dstOpnd, dstType, NULL, false, obj, clz, isLoad);

    return dstOpnd;
}

//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertFpToFp(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, Class_Handle clz, bool isLoad)
{
    Type * srcType=srcOpnd->getType();
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertToUnmanagedPtr(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj, Class_Handle clz, bool isLoad) {
    ...
    if (srcType->isObject()) {
        ...
        OpndSize dstSize=irManager.getTypeSize(dstType);
        if (srcSize == dstSize) {
            Inst* inst = irManager.newCopyPseudoInst(Mnemonic_MOV, dstOpnd, srcOpnd);
            if (isLoad && obj)
                 inst->setParentObjectLoad(obj);
            else if (obj)
                 inst->setParentObjectStore(obj);
            ...
            if (srcType->isInteger()) {
                Inst* inst = irManager.newInstEx(isZeroExtend?Mnemonic_MOVZX:Mnemonic_MOVSX, 1, dstOpnd, srcOpnd);
                if (isLoad && obj)
                     inst->setParentObjectLoad(obj);
                else if (obj)
                     inst->setParentObjectStore(obj);
        ...
        convertIntToInt(srcOpnd, dstType, dstOpnd, false, obj, clz, isLoad);
    } else {
        assert(0);
    }
    return dstOpnd;
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertUnmanagedPtr(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, Class_Handle clz, bool isLoad) {
    Type * srcType=srcOpnd->getType();
    ...
    if (dstType->isObject()) {
        ...
        if (dstSize<=srcSize) {
            ...
            assert(dstSize==OpndSize_64);
#ifdef _EM64T_
            Inst* inst = irManager.newInstEx(srcType->isSignedInteger()?Mnemonic_MOVSX:Mnemonic_MOVZX, 1, dstOpnd, srcOpnd);
            if (isLoad && obj)
                 inst->setParentObjectLoad(obj);
            else if (obj)
                 inst->setParentObjectStore(obj);
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convert(CG_OpndHandle * oph, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj, Class_Handle clz, bool isLoad)
{
    //debugging instruction
    if (Log::isEnabled()){
    if (obj)
    {
        Log::out() << "[convert opnd: ";
        Opnd::DefScope ds=obj->getDefScope();
        Log::out()<<(
            ds==Opnd::DefScope_Variable?"v":
            ds==Opnd::DefScope_SemiTemporary?"s":
    ...
    if (isIntegerType(srcType)){
        if (isIntegerType(dstType)){
            dstOpnd=convertIntToInt(srcOpnd, dstType, dstOpnd, isZeroExtend, obj, clz, isLoad);
            converted=true;
        } else if (dstType->isFP()){
            dstOpnd=convertIntToFp(srcOpnd, dstType, dstOpnd, obj, clz, isLoad);
            converted=true;
        } else if (dstType->isUnmanagedPtr()) {
            dstOpnd = convertToUnmanagedPtr(srcOpnd, dstType, dstOpnd, isZeroExtend, obj, clz, isLoad);
            converted = true;
        }
    }else if (srcType->isFP()){
        if (dstType->isInteger()){
            dstOpnd=convertFpToInt(srcOpnd, dstType, dstOpnd, obj, clz, isLoad);
            converted=true;
        }else if (dstType->isFP()){
            dstOpnd=convertFpToFp(srcOpnd, dstType, dstOpnd, obj, clz, isLoad);
            converted=true;
        }   
    } else if (srcType->isUnmanagedPtr() && !dstType->isUnmanagedPtr()) {
        dstOpnd = convertUnmanagedPtr(srcOpnd, dstType, dstOpnd, obj, clz, isLoad);
        converted = true;
    } else  if (srcType->isObject() && dstType->isUnmanagedPtr()) {
        dstOpnd = convertToUnmanagedPtr(srcOpnd, dstType, dstOpnd, false, obj, clz, isLoad);
        converted = true;
    }
    if (!converted){
...
#ifdef RECORD_STATIC_IN_FIELD
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertIntToInt(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj, 
                                                                                  Class_Handle clz, Field_Handle field, bool isLoad)
{
    Type * srcType=srcOpnd->getType();
    assert(isIntegerType(srcType) && (isIntegerType(dstType) || dstType->isPtr()));
    ...
    if (dstSize==srcSize){ // trivial conversion
        if (dstOpnd==NULL && dstType == srcType) {
            ...
            copyOpndTrivialOrTruncatingConversion(dstOpnd, srcOpnd, obj, clz, field, isLoad);
        }
    }else if (dstSize<=srcSize){ // truncating conversion 
        if (dstOpnd==NULL)
            dstOpnd=irManager.newOpnd(dstType);
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertIntToFp(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, 
                                                                                               Class_Handle clz, Field_Handle field, bool isLoad)
{
    assert(srcOpnd->getType()->isInteger() && dstType->isFP());
    if (dstOpnd==NULL) {
    ...
    if (srcSize<OpndSize_32) {
        srcOpnd=convert(srcOpnd, typeManager.getInt32Type(), NULL, false, obj, clz, field, isLoad);
        srcSize = srcOpnd->getSize();
    }
    assert(srcSize == OpndSize_32 || srcSize == OpndSize_64);
    ...
    if (!useHelpers) { //use SSE
        if (srcSize == OpndSize_32) {
            ...
            if (isLoad && obj)
                 ...
                 convInst->setAccessFieldLoad(field);
            else
                 convInst->setAccessFieldStore(field);
            appendInsts(convInst);
        } else { 
            //use FPU to convert long to float/double on IA32 platform
#ifdef _EM64T_
            ...
            if (isLoad && obj)
                 ...
                 copyInst->setAccessFieldLoad(field);
            else
                 copyInst->setAccessFieldStore(field);
            appendInsts(copyInst);
            ...
            if (isLoad && obj)
                 ...
                 memAliasInst->setAccessFieldLoad(field);
            else
                 memAliasInst->setAccessFieldStore(field);
            appendInsts(memAliasInst);
#endif
            ...
            if (dstType->isDouble()) {
                ...
                if (isLoad && obj)
                     ...
                     copy2SSEInst->setAccessFieldLoad(field);
                else
                     copy2SSEInst->setAccessFieldStore(field);
                appendInsts(copy2SSEInst);
            } else {
                assert(dstSize == OpndSize_32);
                convertFpToFp(fpResOpnd, dstType, dstOpnd, obj, clz, field, isLoad);
            }
        }
    }
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertToUnmanagedPtr(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj,
                                                                                                          Class_Handle clz, Field_Handle field, bool isLoad) {
    assert(dstType->isUnmanagedPtr());
    Type* srcType = srcOpnd->getType();
    if (dstOpnd == NULL) {
    ...
    if (srcType->isObject()) {
        ...
        convertIntToInt(srcOpnd, dstType, dstOpnd, false, obj, clz, field, isLoad);
    } else {
        assert(0);
    }
    return dstOpnd;
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertFpToInt(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, 
                                                                                   Class_Handle clz, Field_Handle field, bool isLoad)
{
    assert(srcOpnd->getType()->isFP() && dstType->isInteger());
    const char * helperName;
    ...
    if (dstSize<OpndSize_32)
        dstOpnd=convert(dstOpnd, dstType, NULL, false, obj, clz, field, isLoad);
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertFpToFp(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, 
                                                                       Class_Handle clz, Field_Handle field, bool isLoad)
{
    Type * srcType=srcOpnd->getType();
    assert(srcType->isFP() && dstType->isFP());
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convertUnmanagedPtr(Opnd * srcOpnd, Type * dstType, Opnd * dstOpnd, Opnd* obj, 
                                                                                  Class_Handle clz, Field_Handle field, bool isLoad) {
    Type * srcType=srcOpnd->getType();
    assert(srcType->isUnmanagedPtr());
...
//_______________________________________________________________________________________________________________
Opnd * InstCodeSelector::convert(CG_OpndHandle * oph, Type * dstType, Opnd * dstOpnd, bool isZeroExtend, Opnd* obj, 
                                                              Class_Handle clz, Field_Handle field, bool isLoad)
{
    //debugging instruction
    if (Log::isEnabled()){
    ...
    if (isIntegerType(srcType)){
        if (isIntegerType(dstType)){
            dstOpnd=convertIntToInt(srcOpnd, dstType, dstOpnd, isZeroExtend, obj, clz, field, isLoad);
            converted=true;
        } else if (dstType->isFP()){
            dstOpnd=convertIntToFp(srcOpnd, dstType, dstOpnd, obj, clz, field, isLoad);
            converted=true;
        } else if (dstType->isUnmanagedPtr()) {
            dstOpnd = convertToUnmanagedPtr(srcOpnd, dstType, dstOpnd, isZeroExtend, obj, clz, field, isLoad);
            converted = true;
        }
    }else if (srcType->isFP()){
        if (dstType->isInteger()){
            dstOpnd=convertFpToInt(srcOpnd, dstType, dstOpnd, obj, clz, field, isLoad);
            converted=true;
        }else if (dstType->isFP()){
            dstOpnd=convertFpToFp(srcOpnd, dstType, dstOpnd, obj, clz, field, isLoad);
            converted=true;
        }   
    } else if (srcType->isUnmanagedPtr() && !dstType->isUnmanagedPtr()) {
        dstOpnd = convertUnmanagedPtr(srcOpnd, dstType, dstOpnd, obj, clz, field, isLoad);
        converted = true;
    } else  if (srcType->isObject() && dstType->isUnmanagedPtr()) {
        dstOpnd = convertToUnmanagedPtr(srcOpnd, dstType, dstOpnd, false, obj, clz, field, isLoad);
        converted = true;
    }
    if (!converted){
        assert(irManager.getTypeSize(dstType)==srcOpnd->getSize());
                                                ...
                                                Field_Handle field) 
{
    ...
    if(irManager.refsAreCompressed() && memType > Type::Float && memType!=Type::UnmanagedPtr) {
        ...
        copyOpnd(dst, opnd, true, obj, clz, field, true);
        // uncompress
...
#endif
    {
        ...
        Opnd * dst = irManager.newOpnd(dstType);
        copyOpnd(dst, opnd, false, obj, clz, field, true);
        return dst;
    }
}
                                                ...
                                                Class_Handle clz) 
{
    ...
    if(irManager.refsAreCompressed() && memType > Type::Float && memType!=Type::UnmanagedPtr) {
        ...
        Opnd * dst = irManager.newOpnd(typeManager.getInt64Type());
        // loading compressed 32-bit managed address, ensure zero-extention
        copyOpnd(dst, opnd, true, obj, clz, true);
        // uncompress
        Type* unmanagedPtrType = typeManager.getUnmanagedPtrType(typeManager.getInt8Type());
        Opnd* heapBase = heapBaseOpnd(unmanagedPtrType, (POINTER_SIZE_INT)VMInterface::getHeapBase());
        dst = simpleOp_I8(Mnemonic_ADD, dstType, dst, heapBase);
...
#endif
    {
        Opnd * opnd = irManager.newMemOpndAutoKind(irManager.getTypeFromTag(memType), addr);
        Opnd * dst = irManager.newOpnd(dstType);
        copyOpnd(dst, opnd, false, obj, clz, true);
        return dst;
    }
}
                                      ...
                                      Field_Handle field) 
{
    ...
    if(irManager.refsAreCompressed() && memType > Type::Float && !src->getType()->isUnmanagedPtr()) {
        ...
        Inst* inst = irManager.newCopyPseudoInst(Mnemonic_MOV, opnd, compressed_src);
...
#endif
    {
        ...
        copyOpnd(dst, src, false, obj, clz, field, false);
    }
} 
#endif
                                      ...
                                      Class_Handle clz) 
{
...
#endif
    {
        assert( !src->getType()->isUnmanagedPtr() || src->getType()->asPtrType()->getPointedToType()->isInt1());
        Opnd * dst = irManager.newMemOpndAutoKind(irManager.getTypeFromTag(memType), addr);
        copyOpnd(dst, src, false, obj, clz, false);
    }
} 
#endif
void InstCodeSelector::simpleStInd(Opnd * addr, 
...
CG_OpndHandle* InstCodeSelector::tau_ldInd(Type* dstType, CG_OpndHandle* ptr, 
                                          ...
                                          Field_Handle field) 
{
    return simpleLdInd(dstType, (Opnd*)ptr, memType, (Opnd*)tauBaseNonNull, (Opnd*)tauAddressInRange, (Opnd*)obj, clz, field);
}
#endif
...
CG_OpndHandle* InstCodeSelector::tau_ldInd(Type* dstType, CG_OpndHandle* ptr, 
                                          ...
                                          Class_Handle clz) 
{
    return simpleLdInd(dstType, (Opnd*)ptr, memType, (Opnd*)tauBaseNonNull, (Opnd*)tauAddressInRange, (Opnd*)obj, clz);
}
#endif
CG_OpndHandle* InstCodeSelector::tau_ldInd(Type* dstType, CG_OpndHandle* ptr, 
                                          Type::Tag memType,
...
void InstCodeSelector::tau_stInd(CG_OpndHandle* src, 
                                    ...
                                    Field_Handle field) 
{
    return simpleStInd((Opnd*)ptr, (Opnd*)src, memType, autoCompressRef, (Opnd*)tauBaseNonNull, (Opnd*)tauElemTypeChecked, (Opnd*) obj, clz, field);
}
#endif
...
void InstCodeSelector::tau_stInd(CG_OpndHandle* src, 
                                    ...
                                    Class_Handle clz) 
{
    return simpleStInd((Opnd*)ptr, (Opnd*)src, memType, autoCompressRef, (Opnd*)tauBaseNonNull, (Opnd*)tauElemTypeChecked, (Opnd*) obj, clz);
}
#endif
void InstCodeSelector::tau_stInd(CG_OpndHandle* src, 
                                    CG_OpndHandle* ptr, 
                                    ...
                                    Field_Handle field)
{
    ...
    CallInst * callInst = irManager.newRuntimeHelperCallInst(
                                        ...
                                        3,helperOpnds,NULL);
    if (obj){
        callInst->setParentObjectStore((Opnd*)obj);
    }else if(field){
        callInst->setAccessFieldStore(field);
    }
                                    ...
                                    Class_Handle clz)
{
    ...
    if (obj)
        ...
        callInst->setParentClassStore(clz);
...
#ifdef THREAD_LOCAL_TAG
//_______________________________________________________________________________________________________________
//  Create new object

CG_OpndHandle* InstCodeSelector::newObj(ObjectType* objType, bool hasThreadLocalObj) 
{
    Opnd * helperOpnds[] = {
    ...
    appendInsts(callInst);
	if(hasThreadLocalObj){
		callInst->setThreadLocalObj(retOpnd);
	}
    return retOpnd;
}
                                              ...
                                              bool hasThreadLocalObj) 
{
    ...
    {
		dstOpnd = retType==NULL ? NULL: irManager.newOpnd(retType);
        dstInst = irManager.newRuntimeHelperCallInst(callId, numArgs, (Opnd**)args, dstOpnd);
        appendInsts(dstInst);
        break;
	}
    default:
	...
	if(hasThreadLocalObj){
		assert(dstInst);
		dstInst->setThreadLocalObj(dstOpnd);
	}
    return dstOpnd;
}
#else
...
#ifdef ORDER
//        callInst->setParentObjectLoad((Opnd*)obj);
#endif
    appendInsts(callInst);
    return dst;
}
