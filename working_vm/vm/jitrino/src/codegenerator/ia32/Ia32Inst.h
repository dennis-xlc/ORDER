...
public:
...
#ifdef ORDER
    bool replaceOpnd(Opnd * opndOld, Opnd * opndNew, U_32 opndRoleMask=OpndRole_All, Inst* inst=NULL);
#else
    ...
    */
#ifdef ORDER
    bool replaceOpnds(Opnd * const * opndMap, U_32 opndRoleMask=OpndRole_All, Inst * const * instMap=NULL);
#else
...
protected:
...
#ifdef ORDER
    void clearParentObjectLoad() {assert(obj_load);obj_load = NULL;}
    void setParentObjectLoad(Opnd* parentObj) {assert(!obj_load);obj_load = parentObj;}
    Opnd* getParentObjectLoad() const {return obj_load;}

    void clearParentObjectStore() {assert(obj_store);obj_store = NULL;}
    void setParentObjectStore(Opnd* parentObj) {assert(!obj_store);obj_store = parentObj;}
    Opnd* getParentObjectStore() const {return obj_store;}

    void clearParentClassLoad() {assert(clz_load);clz_load = NULL;}
    void setParentClassLoad(Class_Handle parentClz) {assert(!clz_load);clz_load = parentClz;}
    Class_Handle getParentClassLoad() const {return clz_load;}

    void clearParentClassStore() {assert(clz_store);clz_store = NULL;}
    void setParentClassStore(Class_Handle parentClz) {assert(!clz_store);clz_store = parentClz;}
    Class_Handle getParentClassStore() const {return clz_store;}

#ifdef THREAD_LOCAL_TAG
	void clearThreadLocalObj() {assert(thread_local_obj); thread_local_obj = NULL;}
	void setThreadLocalObj(Opnd* opnd) {assert(!thread_local_obj); thread_local_obj = opnd;}
	Opnd* getThreadLocalObj() const {return thread_local_obj;}
#endif

#ifdef RECORD_STATIC_IN_FIELD
    void clearAccessFieldLoad() {assert(field_load);field_load = NULL;}
    void setAccessFieldLoad(Field_Handle accessField) {assert(!field_load);field_load = accessField;}
    Field_Handle getAccessFieldLoad() const {return field_load;}

    void clearAccessFieldStore() {assert(field_store);field_store = NULL;}
    void setAccessFieldStore(Field_Handle accessField) {assert(!field_store);field_store = accessField;}
    Field_Handle getAccessFieldStore() const {return field_store;}
#endif
    ...
    Opnd*						obj_load;
    Opnd*						obj_store;
    Class_Handle				clz_load;
    Class_Handle				clz_store;

#ifdef THREAD_LOCAL_TAG
	Opnd* 						thread_local_obj;
#endif

#ifdef RECORD_STATIC_IN_FIELD
	Field_Handle                        field_load;
	Field_Handle                        field_store;
#endif
    ...
    friend class CopyExpansion;
    friend class ORDERRec;
    friend class ORDERRep;
    friend class EarlyPropagation;
    friend class DCE;
#endif
