...
#ifdef ORDER
                   Opnd* obj_load = inst->getParentObjectLoad();
                   if (obj_load)
                   {
                       copySequence->setParentObjectLoad(obj_load);
                   }

                   Opnd* obj_store = inst->getParentObjectStore();
                   if (obj_store)
                   {
                       copySequence->setParentObjectStore(obj_store);
                   }

                   Class_Handle clz_load = inst->getParentClassLoad();
                   if (clz_load)
                   {
                       copySequence->setParentClassLoad(clz_load);
                   }

                   Class_Handle clz_store = inst->getParentClassStore();
                   if (clz_store)
                   {
                       copySequence->setParentClassStore(clz_store);
                   }

                   Field_Handle field_load = inst->getAccessFieldLoad();
                   if (field_load)
                   {
                       copySequence->setAccessFieldLoad(field_load);
                   }	

                   Field_Handle field_store = inst->getAccessFieldStore();
                   if (field_store)
                   {
                       copySequence->setAccessFieldStore(field_store);
                   }
