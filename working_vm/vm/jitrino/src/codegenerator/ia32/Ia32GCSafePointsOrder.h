...
class GCPointsBaseLiveRangeFixerOrder: public SessionAction {
public:
    void runImpl();
...
public:
    ...
    OrderGCSafePointsInfo(MemoryManager& mm, IRManager& irm, Mode mode);
   ...
   bool isStaticFieldMptr(Opnd* opnd) const { return staticMptrs.find(opnd)!=staticMptrs.end();}        
    ...
    static bool isOrderGCSafePoint(const Inst* inst)  {return IRManager::isOrderGCSafePoint(inst);}
