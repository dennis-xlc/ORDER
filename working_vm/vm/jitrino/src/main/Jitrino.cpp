...
#ifdef ORDER_DEBUG
int is_in_runPipeline[ORDER_THREAD_NUM] = { 0 };
#endif

void runPipeline(CompilationContext* c) {
#ifdef ORDER_DEBUG
//    printf("running in runPipeline : [%d]\n", (int)pthread_self());
#endif
    globalTimer.start();
...
#ifdef ORDER_DEBUG
//        printf("running in pipeline %s in thread %d\n", sa->getName(), (int)pthread_self());
#endif
