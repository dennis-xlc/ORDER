...
//yzm
//ORDER introduced interface
static  get_replay_access_info_method_t get_replay_access_info_method = 0;
static  get_record_access_info_method_t get_record_access_info_method = 0;
static  vm_enter_method_t vm_enter_method = 0;
static  vm_method_return_t vm_method_return = 0;
static  vm_is_record_t vm_is_record = 0;
static  get_tm_self_tls_addr_t  get_tm_self_tls_addr = 0;
static  is_field_final_t is_field_final = 0;
static  is_initializing_t is_initializing = 0;
static  get_tid_offset_t  get_tid_offset = 0;
static  printf_addr_t  printf_addr = 0;
static  record_System_Call_In_Compile_t record_System_Call_In_Compile = 0;
static  get_System_Call_In_Compile_t get_System_Call_In_Compile = 0;
static  vm_get_managed_null_object_t  vm_get_managed_null_object = 0; //vm_get_managed_null_object
static  is_lazy_resolution_enable_t  is_lazy_resolution_enable = 0;
static  is_show_parse_process_t  is_show_parse_process = 0;
#ifdef RECORD_STATIC_IN_FIELD
static  get_record_field_access_info_method_t get_record_field_access_info_method = 0;
#endif
//static  vm_enter_sleep_t vm_enter_sleep = 0;
...
#define GET_INTERFACE(get_adapter, func_name) \
    ...
    bool VMInterface::setVmAdapter(vm_adaptor_t a) {
        ...
        //yzm
        //ORDER introduced interfaces
        get_replay_access_info_method = GET_INTERFACE(vm, get_replay_access_info_method);
        get_record_access_info_method = GET_INTERFACE(vm, get_record_access_info_method);
        vm_enter_method = GET_INTERFACE(vm, vm_enter_method);
        vm_method_return = GET_INTERFACE(vm, vm_method_return);
        vm_is_record = GET_INTERFACE(vm, vm_is_record);
        get_tm_self_tls_addr = GET_INTERFACE(vm, get_tm_self_tls_addr);
        is_field_final = GET_INTERFACE(vm, is_field_final);
        is_initializing = GET_INTERFACE(vm, is_initializing);
        get_tid_offset = GET_INTERFACE(vm, get_tid_offset);
        printf_addr = GET_INTERFACE(vm, printf_addr);
        record_System_Call_In_Compile = GET_INTERFACE(vm, record_System_Call_In_Compile);
        get_System_Call_In_Compile = GET_INTERFACE(vm, get_System_Call_In_Compile);
        vm_get_managed_null_object = GET_INTERFACE(vm, vm_get_managed_null_object);
        is_lazy_resolution_enable = GET_INTERFACE(vm, is_lazy_resolution_enable);
        is_show_parse_process = GET_INTERFACE(vm, is_show_parse_process);
//        vm_enter_sleep = GET_INTERFACE(vm, vm_enter_sleep);

#ifdef RECORD_STATIC_IN_FIELD
        get_record_field_access_info_method = GET_INTERFACE(vm, get_record_field_access_info_method);
#endif
...
#ifdef RECORD_STATIC_IN_FIELD
void*
VMInterface::getRecordFieldAccessInfoMethod()
{
    return get_record_field_access_info_method();
}
#endif
...
//ORDER
void*
VMInterface::getReplayAccessInfoMethod()
{
    return get_replay_access_info_method();
}

void*
VMInterface::getRecordAccessInfoMethod()
{
    return get_record_access_info_method();
}

void*
VMInterface::vmEnterMethod()
{
    return vm_enter_method();
}

void*
VMInterface::vmMethodReturn()
{
    return vm_method_return();
}

bool
VMInterface::vmOrderRecord()
{
    return vm_is_record();
}

void
VMInterface::vm_Record_System_Call_In_Compile(U_32 index, bool isUnresolved)
{
    record_System_Call_In_Compile(index, isUnresolved);
}


bool
VMInterface::vm_Get_System_Call_In_Compile(U_32 index)
{
    return get_System_Call_In_Compile(index);
}

bool
VMInterface::vm_is_lazy_resolution_enable()
{
    return is_lazy_resolution_enable();
}


bool
VMInterface::vm_is_show_parse_process()
{
    return is_show_parse_process();
}
/*
void*
VMInterface::vmEnterSleep()
{
    return vm_enter_sleep();
}
*/

void*
VMInterface::getTIDAddress()
{
    return get_tm_self_tls_addr();
}

bool
VMInterface::isInitializing()
{
...
unsigned
VMInterface::getTIDOffset()
{
    return get_tid_offset();
}

bool
VMInterface::isFieldFinal(Class_Handle cl, U_16 index)
{
    return is_field_final(cl, index);
}
...
#ifdef ORDER_DEBUG
//    printf("Enter getNonStaticField, thread %d\n", (int)pthread_self());	
#endif
#ifdef ORDER
    if(VMInterface::vm_is_lazy_resolution_enable()){
        bool lazy = typeManager.isLazyResolutionMode();
        assert(lazy == TRUE);
        if (!lazy || class_cp_is_entry_resolved(enclClass, cpIndex)) {
        ...
        if (!res) {
#ifdef ORDER_DEBUG
 //           printf("Exit getNonStaticField, thread %d\n", (int)pthread_self());
#endif
            return NULL;
        }
    }
...
CompilationInterface::getStaticField(Class_Handle enclClass, U_32 cpIndex, bool putfield) {
    Field_Handle res = NULL;
#ifdef ORDER_DEBUG
//    printf("Enter getStaticField, thread %d\n", (int)pthread_self());
#endif
#if (defined(ORDER) && (defined(RECORD_STATIC) || defined(RECORD_STATIC_IN_FIELD)))
    {
#else
    bool lazy = typeManager.isLazyResolutionMode();
    if (!lazy || class_cp_is_entry_resolved(enclClass, cpIndex)) {
