...
#ifdef THREAD_LOCAL_TAG
static bool hasThreadLocalTag(Method_Handle method_handle, U_32 _pc){
	if(method_handle->get_thread_local_table_size() != 0){
		jlong pc;
		for(unsigned i = 0; i < method_handle->get_thread_local_table_size(); i++){
			method_handle->get_thread_local_entry(i, &pc);
			//printf("[Method %s:%s]: find a Thread-Local tag at pc %d\n", 
			//	method_handle->get_class()->get_name()->bytes, method_handle->get_name()->bytes, (int)pc);
			if(((U_32)pc) == _pc)
				return true;
		}
	}
...
//        Class_Handle cur_clz= methodToCompile.getParentHandle();
        ...
        res = irBuilder.genLdStaticWithResolve(fieldType, methodToCompile.getParentType()->asObjectType(), constPoolIndex, methodToCompile.getParentHandle());
#else
...
//        Class_Handle cur_clz= methodToCompile.getParentHandle();
        ...
        irBuilder.genStStaticWithResolve(fieldType, methodToCompile.getParentType()->asObjectType(), constPoolIndex, value, methodToCompile.getParentHandle());
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::getfield(U_32 constPoolIndex, U_32 pc)
#else
void 
...
#ifdef THREAD_LOCAL_TAG
	//if(VMInterface::vmOrderRecord())
		if(hasThreadLocalTag(methodToCompile.getMethodHandle(), pc)){
			field->setThreadLocal(true);
		}
#endif
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::putfield(U_32 constPoolIndex, U_32 pc)
#else
void 
...
#ifdef THREAD_LOCAL_TAG
void JavaByteCodeTranslator::iaload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
	genArrayLoad(typeManager.getInt32Type(), isThreadLocal);  
}
void JavaByteCodeTranslator::laload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getInt64Type(), isThreadLocal);
}
void JavaByteCodeTranslator::faload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getSingleType(), isThreadLocal);
}
void JavaByteCodeTranslator::daload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getDoubleType(), isThreadLocal);
}
void JavaByteCodeTranslator::aaload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genTypeArrayLoad(isThreadLocal);
}
void JavaByteCodeTranslator::baload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getInt8Type(), isThreadLocal);
}
void JavaByteCodeTranslator::caload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getCharType(), isThreadLocal);
}
void JavaByteCodeTranslator::saload(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayLoad(typeManager.getInt16Type(), isThreadLocal);
}
void JavaByteCodeTranslator::iastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getInt32Type(), isThreadLocal);
}
void JavaByteCodeTranslator::lastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getInt64Type(), isThreadLocal);
}
void JavaByteCodeTranslator::fastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getSingleType(), isThreadLocal);
}
void JavaByteCodeTranslator::dastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getDoubleType(), isThreadLocal);
}
void JavaByteCodeTranslator::aastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genTypeArrayStore(isThreadLocal);
}
void JavaByteCodeTranslator::bastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getInt8Type(), isThreadLocal);
}
void JavaByteCodeTranslator::castore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getCharType(), isThreadLocal);
}
void JavaByteCodeTranslator::sastore(U_32 pc) {
	bool isThreadLocal = false;
//	if(VMInterface::vmOrderRecord())
		isThreadLocal = hasThreadLocalTag(methodToCompile.getMethodHandle(), pc);
    genArrayStore(typeManager.getInt16Type(), isThreadLocal);
}
#else
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::new_(U_32 constPoolIndex, U_32 pc) {
    NamedType* type = compilationInterface.getNamedType(methodToCompile.getParentHandle(), constPoolIndex, ResolveNewCheck_DoCheck);
    jitrino_assert(type);
	Opnd* newObjOpnd = NULL;
	
#ifdef ORDER
    bool unsolved = type->isUnresolvedObject();

    if(VMInterface::vmOrderRecord()){
        VMInterface::vm_Record_System_Call_In_Compile(constPoolIndex, unsolved);
    }
    else{
        bool is_unresolved = VMInterface::vm_Get_System_Call_In_Compile(constPoolIndex);
        if(is_unresolved == false){
            assert(type->isUnresolvedObject() == false);
/*
            while(type->isUnresolvedObject())
            {
                usleep(1000);
#ifdef ORDER_DEBUG
                printf("usleep in code translator : %d\n", (U_32)hythread_self()->thread_id);
#endif
                hythread_yield();
                hythread_safe_point();
                hythread_exception_safe_point();

                type = compilationInterface.getNamedType(methodToCompile.getParentHandle(), constPoolIndex, ResolveNewCheck_DoCheck);
            }
*/
            unsolved = false;
        }
        else if(is_unresolved == 1){
            assert(type->isUnresolvedObject() == true);
            unsolved = true;
        }
        else{ assert(0);}
    ...
    if (type->isUnresolvedObject()) {
        ...
        newObjOpnd = irBuilder.genNewObjWithResolve(methodToCompile.getParentType()->asObjectType(), constPoolIndex);
    } else {
#ifdef _DEBUG
        const char* typeName = type->getName();
        assert(!VMMagicUtils::isVMMagicClass(typeName));
#endif
        newObjOpnd = irBuilder.genNewObj(type);
    }
	...
	//if(VMInterface::vmOrderRecord())
		if(hasThreadLocalTag(methodToCompile.getMethodHandle(), pc))
			newObjOpnd->getInst()->setThreadLocalObject(newObjOpnd);
}
void 
JavaByteCodeTranslator::newarray(U_8 atype, U_32 pc) {
    NamedType* type = NULL;
    switch (atype) {
    case 4:    // boolean
	...
	if(VMInterface::vmOrderRecord())
		if(hasThreadLocalTag(methodToCompile.getMethodHandle(), pc))
			arrayOpnd->getInst()->setThreadLocalObject(arrayOpnd);
...
void 
JavaByteCodeTranslator::anewarray(U_32 constPoolIndex, U_32 pc) {
	Opnd* newObjOpnd = NULL;
    ...
    if (type->isUnresolvedType()) {
        ...
        newObjOpnd = irBuilder.genNewArrayWithResolve(type, sizeOpnd, methodToCompile.getParentType()->asObjectType(), constPoolIndex);
    } else {
        newObjOpnd = irBuilder.genNewArray(type,sizeOpnd);
    }
...
void 
JavaByteCodeTranslator::multianewarray(U_32 constPoolIndex,U_8 dimensions, U_32 pc) {
	Opnd* newObjOpnd = NULL;
    ...
    if (arraytype->isUnresolvedType()) {
        ...
        newObjOpnd = irBuilder.genMultianewarrayWithResolve(
            arraytype, methodToCompile.getParentType()->asObjectType(),constPoolIndex, dimensions,countOpnds
            );
    } else {
        newObjOpnd = irBuilder.genMultianewarray(arraytype,dimensions,countOpnds);
    }
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::genArrayLoad(Type* type, bool isThreadLocal)
#else
void 
...
#endif
{
    ...
    Opnd* base = popOpnd();
#ifdef THREAD_LOCAL_TAG
    pushOpnd(irBuilder.genLdElem(type,base,index,isThreadLocal));
#else
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::genTypeArrayLoad(bool isThreadLocal)
#else
void 
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::genArrayStore(Type* type, bool isThreadLocal)
#else
void 
...
#endif
{
    ...
    Opnd*    base = popOpnd();
#ifdef THREAD_LOCAL_TAG
    irBuilder.genStElem(type,base,index,value,isThreadLocal);
#else
...
#ifdef THREAD_LOCAL_TAG
void 
JavaByteCodeTranslator::genTypeArrayStore(bool isThreadLocal) 
#else
void 
...
bool JavaByteCodeTranslator::genVMMagic(const char* mname, U_32 numArgs, Opnd **srcOpnds, Type *magicRetType) {
...
#ifdef THREAD_LOCAL_TAG
        irBuilder.genStElem(opType, arg0, arg1, arg2, tauSafe, tauSafe, tauSafe, false); 
#else
    ...
    if (!strcmp(mname, "get")) {
        assert(numArgs == 2);
#ifdef THREAD_LOCAL_TAG
		Opnd* res = irBuilder.genLdElem(resType, arg0, arg1, tauSafe, tauSafe, false);
#else
