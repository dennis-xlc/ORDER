...
#ifdef ORDER //lijian try to modify define use
	if (useInst->isMultiSrcInst() && ((MultiSrcInst*)useInst)->getParentObject())
	{
		Opnd* src = ((MultiSrcInst*)useInst)->getParentObject();
		addDefUse(src->getInst(),useInst,20);
	}
#endif
