...
//yzm
//used to debug in ORDER
#ifdef ORDER
	if (this->getOpcode() == Op_TauLdInd || 
		this->getOpcode() == Op_TauStInd ||
		this->getOpcode() == Op_TauStRef)
	{
	      assert(this->isMultiSrcInst());
		MultiSrcInst* inst = (MultiSrcInst*)this;
		Opnd* obj = inst->getParentObject();
              if (obj)
              {
                  os << " [opnd: ";
                  if (obj->isGlobal()) 
		...
		Class_Handle clz = inst->getParentClass();
              if (clz)
              {
                  os << " [clz: " << (long) clz << "]";
              }
