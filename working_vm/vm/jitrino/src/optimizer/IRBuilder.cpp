...
/*#if defined(ORDER) && defined(THREAD_LOCAL_TAG)
typedef struct _ManagedObjectDummy {
    ...
    static POINTER_SIZE_INT counter_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->access_count); }
    static POINTER_SIZE_INT tls_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->access_tid); }
    static POINTER_SIZE_INT lock_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->access_lock); }

    static POINTER_SIZE_INT alloc_tid_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->alloc_tid); }
    static POINTER_SIZE_INT alloc_count_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->alloc_count); }

	static POINTER_SIZE_INT order_padding_offset() { return (POINTER_SIZE_INT)(&((_ManagedObjectDummy*)NULL)->order_padding); }

    VMEXPORT static bool _tag_pointer;
} _ManagedObjectDummy;

#define OBJ_THREAD_LOCAL 0x100
#define OBJ_SIZE_EXTEND 0x200 //used to indicate that we should use obj_end_extend to get the object size
#define OBJ_INIT_ONLY 0x400
...
Opnd*
IRBuilder::genTauLdInd(Modifier mod, Type* type, Type::Tag ldType, Opnd* ptr, 
                       Opnd *tauBaseNonNull, Opnd *tauAddressInRange, FieldDesc* fieldDesc) {
    ptr = propagateCopy(ptr);
    tauBaseNonNull = propagateCopy(tauBaseNonNull);
    tauAddressInRange = propagateCopy(tauAddressInRange);
    Opnd* dst = createOpnd(type);

    MultiSrcInst* inst = (MultiSrcInst*)instFactory->makeTauLdInd(mod, ldType, dst, ptr, 
                                        tauBaseNonNull, tauAddressInRange);

    if(inst->getAccessField() == NULL){
        inst->setAccessField(fieldDesc->getFieldHandle());
    }
...
//yzm
//ORDER: record the accessed object
Opnd*
IRBuilder::genTauLdInd(Modifier mod, Type* type, Type::Tag ldType, Opnd* ptr, 
                       Opnd *tauBaseNonNull, Opnd *tauAddressInRange, Opnd* obj) {
    ptr = propagateCopy(ptr);
    tauBaseNonNull = propagateCopy(tauBaseNonNull);
    tauAddressInRange = propagateCopy(tauAddressInRange);
...
//yzm
//ORDER: record the accessed class
Opnd*
IRBuilder::genTauLdInd(Modifier mod, Type* type, Type::Tag ldType, Opnd* ptr, 
                       Opnd *tauBaseNonNull, Opnd *tauAddressInRange, Class_Handle clz) {
    ptr = propagateCopy(ptr);
    tauBaseNonNull = propagateCopy(tauBaseNonNull);
    tauAddressInRange = propagateCopy(tauAddressInRange);
...
#ifdef ORDER
		//yzm
		//ORDER: record the accessed object in instruction
	
		if(!fieldDesc->isInitOnly()) {//not a final field
#ifdef THREAD_LOCAL_TAG
			if(fieldDesc->isThreadLocal()){
				//printf("bypass thread local field load\n");
        	...
        	return genTauLdInd(mod, type, type->tag, 
                           genLdFieldAddr(type, base, fieldDesc), 
                           tauNullCheck, tauAddressInRange,base);
		}
		else{//a final field
			return genTauLdInd(mod, type, type->tag, 
                           genLdFieldAddr(type, base, fieldDesc), 
...
#ifdef ORDER
    return genTauLdInd(mod, type, type->tag, addr,  tauNullCheck, tauAddressInRange, base);
#else
...
#ifdef ORDER //yzm
Opnd*
IRBuilder::genLdStaticWithResolve(Type* type, ObjectType* enclClass, U_32 cpIdx, Class_Handle clz) {
    bool uncompress = false;
    if (irBuilderFlags.compressedReferences && type->isObject()) {
        assert(!type->isCompressedReference());
        uncompress = true;
    ...
    return genTauLdInd(mod, type, type->tag, addrOpnd, tauOk, tauOk, clz);
}
#else
Opnd*
...
IRBuilder::genLdStatic(Type* type, FieldDesc* fieldDesc) {
...
#ifdef RECORD_STATIC_IN_FIELD
            return genTauLdInd(mod, type, type->tag, genLdStaticAddr(type, fieldDesc),
                           tauOk, tauOk, fieldDesc);            
#else
            return genTauLdInd(mod, type, type->tag, genLdStaticAddr(type, fieldDesc),
                           tauOk, tauOk, fieldDesc->getParentHandle());
#endif
        }
#else
        return genTauLdInd(mod, type, type->tag, genLdStaticAddr(type, fieldDesc),
    ...
    Opnd* dst = createOpnd(type);
#ifdef ORDER //yzm
    MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeLdStatic(mod, type, dst, fieldDesc);
    if(!fieldDesc->isInitOnly()){
#ifdef RECORD_STATIC_IN_FIELD
        if(inst->getAccessField() == NULL){
            inst->setAccessField(fieldDesc->getFieldHandle());
...
#else
        inst->setParentClass(fieldDesc->getParentHandle());
#endif
    }
    appendInst(inst);
#else
...
#ifdef THREAD_LOCAL_TAG	
Opnd*
IRBuilder::genLdElem(Type* type, Opnd* array, Opnd* index, Opnd* tauNullChecked, Opnd* tauAddressInRange, bool isThreadLocal) 
#else
Opnd*
...
#endif
{
    ...
    if (irBuilderFlags.expandMemAddrs) {
        ...
        return genTauLdInd(mod, type, type->tag, 
                           genLdElemAddrNoChecks(type, array, index),
                           tauNullChecked, tauAddressInRange, array);
#else
        return genTauLdInd(mod, type, type->tag, 
                           genLdElemAddrNoChecks(type, array, index),
                           tauNullChecked, tauAddressInRange);
    ...
    Opnd* dst = createOpnd(type);
#ifdef ORDER
    MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauLdElem(mod, type, dst, array, index,
                                         tauNullChecked, tauAddressInRange);
#ifdef THREAD_LOCAL_TAG	
	if(!isThreadLocal)
#endif
    inst->setParentObject(array);
    appendInst(inst);
#else
...
#ifdef THREAD_LOCAL_TAG
Opnd*
IRBuilder::genLdElem(Type* type, Opnd* array, Opnd* index, bool isThreadLocal) 
#else
Opnd*
...
#ifdef THREAD_LOCAL_TAG
    return genLdElem(type,array,index,tauNullChecked,tauAddressInRange,isThreadLocal);
#else
...
IRBuilder::genTauStInd(Type* type,
    ...
    if (irBuilderFlags.insertWriteBarriers) {
        MultiSrcInst* inst = (MultiSrcInst*)instFactory->makeTauStInd((Modifier(Store_WriteBarrier)|
                                             compressMod),
                                            type->tag, src, ptr, 
        ...
        MultiSrcInst* inst = (MultiSrcInst*)instFactory->makeTauStInd((Modifier(Store_NoWriteBarrier)|
                                             compressMod),
                                            type->tag, src, ptr,
                                            tauBaseNonNull, tauAddressInRange, tauElemTypeChecked);
        if(inst->getAccessField() == NULL){
...
IRBuilder::genTauStRef(Type* type, Opnd *objectbase, Opnd* ptr, Opnd* src,
                       Opnd *tauBaseNonNull, Opnd *tauAddressInRange,
                       Opnd *tauElemTypeChecked, bool isThreadLocal)
#else
void
IRBuilder::genTauStRef(Type* type, Opnd *objectbase, Opnd* ptr, Opnd* src,
                       Opnd *tauBaseNonNull, Opnd *tauAddressInRange,
...
#endif
{
    ...
    if (irBuilderFlags.insertWriteBarriers) {
#ifdef ORDER //yzm
        MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStRef((Modifier(Store_WriteBarrier)|
                                             compressMod),
		...
		if(!isThreadLocal)
#endif
        inst->setParentObject(objectbase);
        appendInst(inst);
#else
...
#ifdef ORDER //yzm
        MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStRef((Modifier(Store_NoWriteBarrier)|
                                             compressMod),
                                            type->tag, src, ptr, objectbase,
                                            tauBaseNonNull, tauAddressInRange, 
                                            tauElemTypeChecked);
...
#if defined(ORDER) && defined(THREAD_LOCAL_TAG)
			if(fieldDesc->isThreadLocal())
				//printf("bypass thread local field store\n");
            genTauStRef(type, base, ptr, src, 
                        tauBaseNonNull, 
                        tauBaseTypeIsOk,
                        tauStoredTypeIsOk, 
                        fieldDesc->isThreadLocal());
#else
			genTauStRef(type, base, ptr, src, 
                        tauBaseNonNull, 
                        tauBaseTypeIsOk,
...
#ifdef ORDER //yzm
            MultiSrcInst* inst = (MultiSrcInst*)instFactory->makeTauStField((Modifier(Store_WriteBarrier)|
...
#ifdef THREAD_LOCAL_TAG
          if(!fieldDesc->isThreadLocal() && !fieldDesc->isInitOnly())
          {
            	inst->setParentObject(base);
          }
              //printf("bypass thread local field store\n");
              appendInst(inst);
#else
...
#ifdef ORDER //yzm
            MultiSrcInst* inst = (MultiSrcInst*)instFactory->makeTauStField((Modifier(Store_NoWriteBarrier)|
                                                   Modifier(AutoCompress_Yes)), 
                                                  type->tag, src, base,
                                                  tauBaseNonNull, 
                                                  tauBaseTypeIsOk,
...
#ifdef ORDER  //yzm
void       
IRBuilder::genStStaticWithResolve(Type* type, ObjectType* enclClass, U_32 cpIdx, Opnd* src, Class_Handle clz) {
    src = propagateCopy(src);
    ...
    Opnd* addr = genLdStaticAddrWithResolve(type, enclClass, cpIdx, true);
        ...
        assert(0);
#endif
    genTauStInd(type, addr, src, tauOk,  tauOk, tauTypeIsOk, clz);
    return;
}
#else
void       
...
//    Type *fieldType = fieldDesc->getFieldType();
    ...
    if (irBuilderFlags.insertWriteBarriers) {
#ifdef ORDER //yzm
        MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStStatic((Modifier(Store_WriteBarrier)|
                                                Modifier(AutoCompress_Yes)),
                                               type->tag, src,
                                               tauTypeIsOk,
                                               fieldDesc);
...
#ifdef ORDER //yzm
        MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStStatic((Modifier(Store_NoWriteBarrier)|
                                                Modifier(AutoCompress_Yes)), 
                                               type->tag, src,
                                               tauTypeIsOk,
                                               fieldDesc);
    ...
    if (irBuilderFlags.expandMemAddrs) {
        ...
        if (irBuilderFlags.insertWriteBarriers && elemType->isObject()) {
            ...
            genTauStRef(elemType, array, ptr, src, tauNullChecked, tauAddressInRange,
                        tauElemTypeChecked, isThreadLocal);
#else
			genTauStRef(elemType, array, ptr, src, tauNullChecked, tauAddressInRange,
                        tauElemTypeChecked);
#endif
        ...
        if (irBuilderFlags.insertWriteBarriers) {
#ifdef ORDER //yzm
            MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStElem((Modifier(Store_WriteBarrier)|
                                                  Modifier(AutoCompress_Yes)), 
...
#ifdef ORDER //yzm
            MultiSrcInst* inst = (MultiSrcInst*) instFactory->makeTauStElem((Modifier(Store_NoWriteBarrier)|
                                                  Modifier(AutoCompress_Yes)), 
                                                 elemType->tag, src, array, index,
                                                 tauNullChecked, 
                                                 tauAddressInRange,
...
#if defined(ORDER) && defined(THREAD_LOCAL_TAG)
    genStElem(elemType,array,index,src,tauNullChecked,tauBaseTypeChecked,tauAddressInRange,isThreadLocal);
#else
...
/*#if defined(ORDER) && defined(THREAD_LOCAL_TAG)
void IRBuilder::appendThreadLocalBit(Opnd* objOpnd)
{
    assert(objOpnd);
	IRManager *irm = getIRManager();
	Opnd* offset = genLdConstant((I_32)_ManagedObjectDummy::order_padding_offset());
    Opnd* obj = objOpnd;
	Opnd* thread_local_addr = createOpnd(typeManager->getTauType());
    appendInst(instFactory->makeAddOffset(thread_local_addr, obj, offset));

    //Opnd* thread_local_bit_orig = irm->newMemOpnd(typeInt32, MemOpndKind_Any, thread_local_addr); 
	//Opnd* thread_local_bit_res = irm->newMemOpnd(typeInt32, MemOpndKind_Any, thread_local_addr);
    Opnd* thread_local_bit_num = genLdConstant((I_32)OBJ_THREAD_LOCAL);
	Opnd* thread_local_new_val = createOpnd(typeManager->getTauType());

	appendInst(instFactory->makeOr(thread_local_new_val, thread_local_bit_num, thread_local_bit_orig));
	appendInst(instFactory->makeCopy(thread_local_bit_res, thread_local_new_val));
}
#endif
*/
} //namespace Jitrino 
