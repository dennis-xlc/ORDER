...
#ifdef ORDER //lijian patch use for obj opnd
			if(index == 20)
			{
#ifdef ORDER_DEBUG
				assert(useInst->isMultiSrcInst() && ((MultiSrcInst*)useInst)->getParentObject());
#endif
				((MultiSrcInst*)useInst)->setParentObject(patchdef->getDst());
			}
			else
			{
