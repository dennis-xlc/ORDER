        ...
        case Op_DirectCall:
            {
                ...
                assert(!inst->isMultiSrcInst() ||
                          (!((MultiSrcInst*)inst)->getParentObject() && 
                          !((MultiSrcInst*)inst)->getParentClass()));
#endif
        ...
        case Op_VMHelperCall:
            {
				...
				cgInst = 
                	instructionCallback.callvmhelper(inst->getNumSrcOperands(),
                                                 ...
                                                 inst->getDst()->getType(),
                                                 callId, 
                                                 inst->getThreadLocalObject() ? true : false);
#else
                cgInst = 
                    instructionCallback.callvmhelper(inst->getNumSrcOperands(),
                                                     genCallArgs(call,0),
...
//yzm
//ORDER: pass the operand from HIR to LIR
#ifdef ORDER

#ifdef RECORD_STATIC_IN_FIELD
                Opnd* obj = ((MultiSrcInst*)inst)->getParentObject();
                Field_Handle field = ((MultiSrcInst*)inst)->getAccessField();

                if(!obj && !field){
#ifdef ORDER_DEBUG
		      printf("here the parent object and class does not exist!!!\n");
#endif
                    ...
                    CG_OpndHandle *res = getCGInst(obj);
...
#else  //NOT define RECORD_STATIC_IN_FIELD

                Opnd* obj = ((MultiSrcInst*)inst)->getParentObject();
                Class_Handle clz = ((MultiSrcInst*)inst)->getParentClass();
