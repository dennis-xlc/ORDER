...
public:
	...
	void setThreadLocalObject(Opnd* obj)
	{threadLocalObj = obj;}
	Opnd* getThreadLocalObject()
...
#ifdef ORDER
	//yzm
	//ORDER used interface, used to identify the access object
	void setParentObject(Opnd* obj)
	{parentObj = obj;}
	Opnd* getParentObject()
	{return parentObj;}

	//ORDER used interface, used to identify the access class if access static 
	void setParentClass(Class_Handle clz)
	{parentClz = clz;}
	Class_Handle getParentClass()
	{return parentClz;}


#ifdef RECORD_STATIC_IN_FIELD

	void setAccessField(Field_Handle field)
	{accessField = field;}
	Field_Handle getAccessField()
	{return accessField;}
...
protected:
    ...
    MultiSrcInst(Opcode op, Modifier mod, Type::Tag ty, Opnd* dst)
        : Inst(op, mod, ty, dst), extendedSrcs(0), extendedSrcSpace(0), 
			parentObj(0), parentClz(0), accessField(0) {}
    ...
    MultiSrcInst(Opcode op,
        ...
        : Inst(op, mod, ty, dst, nSrcs_),
          	extendedSrcs(srcs_), extendedSrcSpace(nSrcs_), parentObj(0), parentClz(0), accessField(0)
    {		
        switch (nSrcs_) {
        default:    
...
#else //NOT def RECORD_STATIC_IN_FIELD

    MultiSrcInst(Opcode op, Modifier mod, Type::Tag ty, Opnd* dst)
        : Inst(op, mod, ty, dst), extendedSrcs(0), extendedSrcSpace(0), 
			parentObj(0), parentClz(0){}
    ...
    MultiSrcInst(Opcode op,
                 ...
                 Opnd** srcs_)
        : Inst(op, mod, ty, dst, nSrcs_),
          	extendedSrcs(srcs_), extendedSrcSpace(nSrcs_), parentObj(0), parentClz(0)
    {		
        switch (nSrcs_) {
        default:    
        case 2:     srcs[1] = srcs_[1];
...
#endif //define RECORD_STATIC_IN_FIELD
...
#ifdef ORDER
	//yzm
	//ORDER used field, used to record the accessed object
	Opnd* parentObj;

	//ORDER used field, used to record the accessed class if access static
	Class_Handle parentClz;


#ifdef RECORD_STATIC_IN_FIELD
	Field_Handle accessField;
#endif
