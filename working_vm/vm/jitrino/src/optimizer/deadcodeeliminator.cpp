...
//    for (U_32 i=0; i<inst->getNumSrcOperands(); i++) 
    if (inst->isMultiSrcInst() && ((MultiSrcInst*)inst)->getParentObject())
    {
//        Opnd* src = inst->getSrc(i);
        Opnd* src = ((MultiSrcInst*)inst)->getParentObject();
        // only follow ssa use-def links
        SsaOpnd* srcSsaOpnd = src->asSsaOpnd();
        if (srcSsaOpnd != NULL) {
            // use of an ssa operand
...
DeadCodeEliminator::eliminateDeadCode(bool keepEmptyNodes) {
    ...
    if (usedInstWidth) {
...
#ifdef ORDER
            //yzm
            Inst* inst = workSet.popFront();
            markLiveInst(inst,workSet,usefulInstSet,usefulVarSet,
                         usedInstWidth, minInstId, maxInstId);
            markParentInst(inst,workSet,usefulInstSet,usefulVarSet,
                         usedInstWidth, minInstId, maxInstId);
        ...
        while (!workSet.isEmpty()) {
            ...
            //yzm
            Inst* inst = workSet.popFront();
            markLiveInst1(inst,workSet,
                          usefulInstSet, usefulVarSet, minInstId, maxInstId);
            markParentInst1(inst,workSet,
                          usefulInstSet, usefulVarSet, minInstId, maxInstId);
