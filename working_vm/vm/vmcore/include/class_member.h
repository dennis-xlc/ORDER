...
//copy from ORDER, may contain some unnessary fields
    U_32 access_count;
    U_32 alloc_count;//used for class hashcode
    union{
        U_32 access_tid;
        U_32 alloc_count_for_hashcode;
    };
    U_16 alloc_tid;//used for offset
    union{
        U_8 access_lock;
        U_16 order_padding;
    };

    static POINTER_SIZE_INT counter_offset(Field* field) 
    { 
        return (POINTER_SIZE_INT)(&((Field*)field)->access_count) - 
			(POINTER_SIZE_INT)field; 
    }
    static POINTER_SIZE_INT tls_offset(Field* field) 
    { 
        return (POINTER_SIZE_INT)(&((Field*)field)->access_tid) - 
			(POINTER_SIZE_INT)field; 
    }

    static POINTER_SIZE_INT lock_offset(Field* field) 
    { 
        return (POINTER_SIZE_INT)(&((Field*)field)->access_lock) - 
			(POINTER_SIZE_INT)field; 
    }

    static POINTER_SIZE_INT alloc_tid_offset(Field* field) 
    { 
        return (POINTER_SIZE_INT)(&((Field*)field)->alloc_tid) - 
			(POINTER_SIZE_INT)field; 
    }

    static POINTER_SIZE_INT alloc_count_offset(Field* field) 
    { 
        return (POINTER_SIZE_INT)(&((Field*)field)->alloc_count) - 
			(POINTER_SIZE_INT)field; 
    }
#endif
...
struct Thread_Local_Table {
	uint16 length;
	Thread_Local_Entry table[1];
};
#endif
...
#ifdef THREAD_LOCAL_TAG
		if(_thread_local_table)
            size += sizeof(uint16) + _thread_local_table->length*sizeof(Thread_Local_Table);
#endif
...
#ifdef THREAD_LOCAL_TAG
	Thread_Local_Table *_thread_local_table;
	bool _parse_thread_locals(unsigned attr_len, ByteReader &cfs);
#endif
...
#ifdef THREAD_LOCAL_TAG
	unsigned get_thread_local_table_size() {
        return (_thread_local_table) ? _thread_local_table->length : 0;
    }


	bool get_thread_local_entry(unsigned index, jlong* pc) {
	    if (_thread_local_table && index < _thread_local_table->length) {
	        *pc = _thread_local_table->table[index].start_pc;
	        return true;
	    } else {
	        return false;
	    }
