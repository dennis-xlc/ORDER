...
#ifdef ORDER_PER_EVA
void Class::dump_method_count()
{

    if(m_methods != NULL) {
        for (int i = 0; i < m_num_methods; i++){
            if (m_methods[i].get_signature() && m_methods[i].count!=0)
            {
                if (m_methods[i].count > 10000000 || m_methods[i].interleaving > 1000)
                    std::cout << "<Important Method>" << std::endl;
                std::cout << "Method Sig : " << m_methods[i].get_signature()->bytes << std::endl;
                std::cout << "Method Name : " << m_methods[i].get_name()->bytes << std::endl;
                std::cout << "Count : " << m_methods[i].count << std::endl;
                std::cout << "Interleaving : " << m_methods[i].interleaving << std::endl;

                if (m_methods[i].is_synchronized())
                {
                    std::cout << "is_synchronized" << std::endl;
                }
                if (m_methods[i].is_native())
                {
                    std::cout << "is_native" << std::endl;
                }
                if (m_methods[i].is_abstract())
                {
                    std::cout << "is_abstract" << std::endl;
                }
                if (m_methods[i].is_varargs())
                {
                    std::cout << "is_varargs" << std::endl;
                }
                if (m_methods[i].is_bridge())
                {
                    std::cout << "is_bridge" << std::endl;
                }
                if (m_methods[i].is_init())
                {
                    std::cout << "is_init" << std::endl;
                }
                if (m_methods[i].is_clinit())
                {
                    std::cout << "is_clinit" << std::endl;
                }
                if (m_methods[i].is_finalize())
                {
                    std::cout << "is_finalize" << std::endl;
                }
                if (m_methods[i].is_overridden())
                {
                    std::cout << "is_overridden" << std::endl;
                }
                if (m_methods[i].is_nop())
                {
                    std::cout << "is_nop" << std::endl;
                }
            }
        }
    }
...
#ifdef THREAD_LOCAL_TAG
	if (_thread_local_table != NULL)
    {
        STD_FREE(_thread_local_table);
        _thread_local_table = NULL;
    }
#endif
...
#define RI_FILE_WITH_FWRITE

extern FILE *order_system_call_resolved_initialized[];

//Set 1 when resoled. Set 0 when initialized. 
#define RESOLVED_INITIALIZED_BIT 		0x80000000
#define NOT_RESOLVED_INITIALIZED_BIT	0x7fffffff
...
tmp_data_t* tmp_data_struct[ORDER_THREAD_NUM];

static bool is_data_empty(tmp_data_t* tmp){
	return (!tmp || tmp->type == TYPE_NULL);
}

static tmp_data_t* init_data(){
	tmp_data_t* tmp = (tmp_data_t*)malloc(sizeof(tmp_data_t));
	bzero(tmp, sizeof(tmp_data_t));
	tmp->type = TYPE_NULL;
	return tmp;
}

static bool is_data_initialized(tmp_data_t* tmp){
	return (tmp && tmp->type == TYPE_INITIALIZED);
}

static bool is_data_resolved(tmp_data_t* tmp){
	return (tmp && tmp->type == TYPE_RESOLVED);
}
...
bool ConstantPool::is_entry_resolved(uint16 index) const {
        assert(this->is_valid_index(index));
#ifdef LOG_REDUCTION
		assert(this->m_entries[0].tags[index] & ConstantPool::RESOLVED_MASK);
		return true;
#else
#ifdef ORDER_N
    U_32 resolve_stat = 0;
    ...
    if(vm_order_record){
        bool resolved = (m_entries[0].tags[index] & ConstantPool::RESOLVED_MASK) != 0;
        resolve_stat = resolved ? 1 : 0 ;
...
#ifdef ORDER_DEBUG
        fprintf(order_system_call[tid], "[%d] %d ", 6, index);
#endif
    ...
    else
    {
        ...
        uint16 index_temp = 0;
        fscanf(order_system_call[tid], "%d ", &index_temp);
        assert(index_temp == index);
#endif
        fscanf(order_system_call[tid], "%d\n", &resolve_stat);

#ifdef ORDER_DEBUG
//        printf("resolve_stat[%d] : c : %d\n", tid, (int)resolve_stat);
#endif
        ...
        {
            while((this->m_entries[0].tags[index] & ConstantPool::RESOLVED_MASK) == 0){
#ifdef ORDER_DEBUG
		printf("the (this->m_entries[0].tags[index] & ConstantPool::RESOLVED_MASK) != 0!!! \n");
#endif
		usleep(100);
            }
            return true;
    ...
    return (this->m_entries[0].tags[index] & ConstantPool::RESOLVED_MASK) != 0;
#endif
#endif
}

#ifdef ORDER
bool ConstantPool::is_entry_resolved(uint16 index, Class *klass) const {
        assert(this->is_valid_index(index));
    ...
    if(vm_order_record){
		...
		if(!resolve_stat)
		{
			if (order_system_call_resolved_initialized[tid] == NULL)
	        {
	            char name[40];
	            sprintf(name, "SYSTEM_CALL_RES_INIT.%d.log", tid);

	            order_system_call_resolved_initialized[tid] = fopen64(name, "a+");
#ifdef ORDER_DEBUG
            	assert(order_system_call_resolved_initialized[tid]);
#endif
	        }
#ifdef RI_FILE_WITH_FWRITE
			U_32 record_hash_code = RESOLVED_INITIALIZED_BIT | klass->get_hash_code();
			fwrite(&record_hash_code, sizeof(record_hash_code), 1, order_system_call_resolved_initialized[tid]);
			fwrite(&index, sizeof(index), 1, order_system_call_resolved_initialized[tid]);
#else
			fprintf(order_system_call_resolved_initialized[tid], "%d %d\n", RESOLVED_INITIALIZED_BIT | klass->get_hash_code(), ((U_32)index));
#endif
		}
#else
        if (order_system_call[tid] == NULL)
    ...
    else
    {
		...
		if(is_data_empty(tmp_data_struct[tid])){
			if (order_system_call_resolved_initialized[tid] == NULL)
        	{
            	char name[40];
            	sprintf(name, "SYSTEM_CALL_RES_INIT.%d.log", tid);

            	order_system_call_resolved_initialized[tid] = fopen(name, "r");
#ifdef ORDER_DEBUG
            	//assert(order_system_call_resolved_initialized[tid]);
#endif
				if(!order_system_call_resolved_initialized[tid]){
#ifdef ORDER_DEBUG
					printf("[1]There is a log file that is always going to do while: SYSTEM_CALL_RES_INIT.%d.log\n",
						tid);
#endif
					goto do_while_resolved;
				}
        	}

			if (feof(order_system_call_resolved_initialized[tid])){
#ifdef ORDER_DEBUG
				printf("reach the file end: SYSTEM_CALL_RES_INIT.%d.log\n", tid);
#endif
				goto do_while_resolved;
			}
#ifdef RI_FILE_WITH_FWRITE
			fread(&hash_code, sizeof(hash_code), 1, order_system_call_resolved_initialized[tid]);
#else
			fscanf(order_system_call_resolved_initialized[tid], "%d ", &hash_code);
#endif

			if(hash_code & RESOLVED_INITIALIZED_BIT){//is_entry_resolved
				//printf("read a entry which is is_entry_resolved: SYSTEM_CALL_RES_INIT.%d.log\n", tid);
#ifdef RI_FILE_WITH_FWRITE
				fread(&index_temp, sizeof(index_temp), 1, order_system_call_resolved_initialized[tid]);
#else
				U_32 index_32 = 0;
				fscanf(order_system_call_resolved_initialized[tid], "%d\n", &index_32);
				index_temp = index_32;
#endif
			}
			else{//is_initialized
#ifndef RI_FILE_WITH_FWRITE
				fscanf(order_system_call_resolved_initialized[tid], "\n");
#endif
				assert(!tmp_data_struct[tid]);
				tmp_data_struct[tid] = init_data();
				tmp_data_struct[tid]->type = TYPE_INITIALIZED;
				tmp_data_struct[tid]->hash_code = hash_code;
				
				goto do_while_resolved;				
			}
		}
		else if(is_data_resolved(tmp_data_struct[tid])){
			hash_code = tmp_data_struct[tid]->hash_code;
			index_temp = tmp_data_struct[tid]->index;
		}
		else{
			assert(is_data_initialized(tmp_data_struct[tid]));
			goto do_while_resolved;
		}
#ifdef ORDER_DEBUG
		//printf("===========in is_resolved: %d (%d, %d)\n", hash_code, index_temp, index);	
		assert(index_temp);
		assert(hash_code & RESOLVED_INITIALIZED_BIT);
		assert(!tmp_data_struct[tid] || is_data_resolved(tmp_data_struct[tid]));
#endif		
		if(klass->get_hash_code() == (hash_code & ((U_32)NOT_RESOLVED_INITIALIZED_BIT)) && 
			index == index_temp){
			if(tmp_data_struct[tid]){
				free(tmp_data_struct[tid]);
				tmp_data_struct[tid] = NULL;
			}
			return false;
		}
		...
		if(!tmp_data_struct[tid]){
			tmp_data_struct[tid] = init_data();
			tmp_data_struct[tid]->type = TYPE_RESOLVED;
			tmp_data_struct[tid]->hash_code = hash_code;
			tmp_data_struct[tid]->index = index_temp;
		}
...
do_while_resolved:
        ...
        if (feof(order_system_call[tid]))
        {
            //assert(0);
            klass->unlock();
            while(true){
                usleep(1000);
                printf("[SYSCALL_SLEEP]: Thread %d  sleep due to the end of file!!!!!\n", tid);
                hythread_yield();
                hythread_safe_point();
...
/*
    ...
    if(vm_order_record){
        bool resolved = (m_entries[0].tags[index] & ConstantPool::ERROR_MASK) != 0;
        error_stat = resolved ? 1 : 0 ;
        U_32 tid = hythread_self()->thread_id;
    ...
    else
    {
        ...
        uint16 index_temp = 0;
        fscanf(order_system_call[tid], "%d %d\n", &index_temp, &error_stat);
        assert(index_temp == index);
        ...
        {
            while((this->m_entries[0].tags[index] & ConstantPool::ERROR_MASK) == 0){
	       printf("the (this->m_entries[0].tags[index] & ConstantPool::ERROR_MASK) != 0!!! \n");
		usleep(100);
            }
            return true;
        }
...
*/
    bool res = (this->m_entries[0].tags[index] & ConstantPool::ERROR_MASK) != 0;
#ifdef ORDER_DEBUG
    assert(res == FALSE);
#endif
    return res;
    ...
    return (this->m_entries[0].tags[index] & ConstantPool::ERROR_MASK) != 0;
#endif
}


bool Class::is_initialized()
{

#ifdef ORDER
    U_32 initalized_stat = 0;

    if(vm_order_record){
        bool clz_initalized = (this->m_state == ST_Initialized);
        initalized_stat = clz_initalized ? 1 : 0 ;
        U_32 tid = hythread_self()->thread_id;
...
#ifdef RI_FILE_WITH_FWRITE
			U_32 record_hash_code = this->get_hash_code() & NOT_RESOLVED_INITIALIZED_BIT;
			fwrite(&record_hash_code, sizeof(record_hash_code), 1, order_system_call_resolved_initialized[tid]);
#else
			fprintf(order_system_call_resolved_initialized[tid], "%d \n", this->get_hash_code() & NOT_RESOLVED_INITIALIZED_BIT);
#endif
		}
#else
        if (order_system_call[tid] == NULL)
...
#ifdef ORDER_DEBUG
        fprintf(order_system_call[tid], "[%d]", 14);

        fwrite(this->m_name->bytes, sizeof(char), this->m_name->len, order_system_call[tid]);

#endif
        fprintf(order_system_call[tid], "%d\n", initalized_stat);
#endif
        //return this->m_state == ST_Initialized;
        return clz_initalized;
    }
    else
    {
        U_32 tid = hythread_self()->thread_id;
		...
		if(is_data_empty(tmp_data_struct[tid])){
			if (order_system_call_resolved_initialized[tid] == NULL)
	        {
				...
				if(!order_system_call_resolved_initialized[tid]){
#ifdef ORDER_DEBUG
					printf("[2]There is a log file that is always going to do while: SYSTEM_CALL_RES_INIT.%d.log\n",
						tid);
#endif
					goto do_while_initialized;
				}
       		}
			if (feof(order_system_call_resolved_initialized[tid])){
#ifdef ORDER_DEBUG
			...
			if(hash_code & RESOLVED_INITIALIZED_BIT){//is_entry_resolved
				...
				uint16 _index;
#ifdef RI_FILE_WITH_FWRITE
				fread(&_index, sizeof(_index), 1, order_system_call_resolved_initialized[tid]);
#else
				U_32 index_tmp = 0;
				fscanf(order_system_call_resolved_initialized[tid], "%d\n", &index_tmp);
				_index = (uint16)index_tmp;
#endif
				assert(!tmp_data_struct[tid]);
				tmp_data_struct[tid] = init_data();
				tmp_data_struct[tid]->type = TYPE_RESOLVED;
				tmp_data_struct[tid]->hash_code = hash_code;
				tmp_data_struct[tid]->index = _index;
		...
		else if(is_data_initialized(tmp_data_struct[tid])){
			hash_code = tmp_data_struct[tid]->hash_code;
		}
		else{
			assert(is_data_resolved(tmp_data_struct[tid]));
			goto do_while_initialized;
		}
#ifdef ORDER_DEBUG
		//printf("===========in is_initialized: %d\n", hash_code);
        assert(!(hash_code & RESOLVED_INITIALIZED_BIT));
		assert(!tmp_data_struct[tid] || is_data_initialized(tmp_data_struct[tid]));
#endif		

		if(this->get_hash_code() == (hash_code & NOT_RESOLVED_INITIALIZED_BIT)){
...
do_while_initialized:
		
		while(this->m_state != ST_Initialized)
		{
#ifdef ORDER_DEBUG
			printf("wait class initialized in jit_runtime_support.cpp\n");
#endif
			usleep(1000);
                           hythread_yield();
                           hythread_safe_point();
        ...
        char *temp_buf = (char *)malloc((this->m_name->len +1) * sizeof(char));
        fread(temp_buf, sizeof(char), this->m_name->len, order_system_call[tid]);
        //fscanf(order_system_call[tid], "%s ", temp_buf);
        temp_buf[this->m_name->len] = '\0';
        if (strcmp(temp_buf, this->m_name->bytes) != 0)
            assert(0);
...
#ifdef ORDER_DEBUG
        if(tid == 75){
            printf("[SYSCALL][14]%s%d\n", this->m_name->bytes, initalized_stat);
        }
#endif

        if (initalized_stat == 0)
        {
...
#ifdef ORDER
//#ifdef LOG_REDUCTION
U_32 Class::hash_function()// ELF Hash Function
{
	U_32 hash = 0;
	U_32 x	= 0;
	const char *str = this->get_name()->bytes;
