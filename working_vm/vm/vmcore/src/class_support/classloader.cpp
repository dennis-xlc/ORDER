...
#ifdef ORDER
Class* ClassLoader::OrderLoadingClass(Global_Env* UNREF env, const String* className)
{
    Class** pklass = NULL;
    Class* klass = NULL;
    LMAutoUnlock aulock(&m_lock);
...
void ClassLoader::addLoadingClass(Global_Env* UNREF env, const String* className)
{
...
#ifdef ORDER
    if(vm_order_record){
        klass = WaitDefinition(env, classNameString);
        ...
        fprintf(order_system_call[tid], "[%d] ", 20);
#endif
        ...
        if(class_status == 0){
            m_lock._lock();
            LoadingClass* loading = m_loadingClasses->Lookup(classNameString);
            assert(loading != NULL);
            // mark ourselves as defining thread
            loading->ChangeDefinerAndInitiator(get_thread_ptr(), this, classNameString);
            m_lock._unlock();
        }
        else if(class_status == 1){
            while((klass = OrderLoadingClass(env, classNameString)) == NULL){
#ifdef ORDER_DEBUG
                printf("[ORDER]: usleep in DoLoadClass!!!\n");
#endif
                usleep(100);
            }
            return klass;
...
#ifdef ORDER_DEBUG
    printf("[DEFINE]: thread %d define class %s\n", hythread_self()->thread_id, classNameString->bytes);
#endif
...
#ifdef ORDER_PER_EVA
void ClassLoader::dump_method_count()
{
    std::map<const String*, Class*>::iterator itr  =  m_loadedClasses->begin();
    for(  ;  itr  !=  m_loadedClasses->end();  ++itr )
    {
        Class* clz = (Class*)itr->second;
        if (clz && clz->get_signature())
        {
            std::cout  << "Class Name : "  << clz->get_name()->bytes << std::endl;
            std::cout  << "Class Sig : "  << clz->get_signature()->bytes << std::endl;
            clz->dump_method_count();
        }
    }

    itr  =  m_initiatedClasses->begin();
    for(  ;  itr  !=  m_initiatedClasses->end();  ++itr )
    {
        Class* clz = (Class*)itr->second;
        if (clz && clz->get_signature())
        {
...
/*
    itr  =  m_loadingClasses->begin();
    for(  ;  itr  !=  m_loadingClasses->end();  ++itr )
    {
        Class* clz = (Class*)itr->second;
        if (clz && clz->get_signature())
        {
...
*/
    itr  =  m_reportedClasses->begin();
    for(  ;  itr  !=  m_reportedClasses->end();  ++itr )
    {
        Class* clz = (Class*)itr->second;
        if (clz && clz->get_signature())
        {
...
//debug used
    printf("[CLASS_OBJ_MAPPING]: Class name : %s  ==> Object[%d, %d]\n", name->bytes, new_java_lang_Class->alloc_tid, new_java_lang_Class->alloc_count);
//end
                                       ...
                                       const String* className)
{
    ...
    if(vm_order_record){
        ...
        klass = StartLoadingClass(m_env, className);
        U_32 tid = hythread_self()->thread_id;
        if (order_system_call[tid] == NULL)
        {
            char name[40];
        ...
        fprintf(order_system_call[tid], "[%d]", 17);

        fwrite(className->bytes, sizeof(char), className->len, order_system_call[tid]);

//        char blank = ' ';
//        fwrite(&blank, sizeof(char), 1, order_system_call[tid]);
        ...
        char *temp_buf = (char *)malloc((className->len +1) * sizeof(char));
        fread(temp_buf, sizeof(char), className->len, order_system_call[tid]);
        //fscanf(order_system_call[tid], "%s ", temp_buf);
        temp_buf[className->len] = '\0';
        if (strcmp(temp_buf, className->bytes) != 0)
            assert(0);
...
//            klass = StartLoadingClass(m_env, className);
...
#endif
                usleep(100);
                klass = OrderLoadingClass(env, className);
            }
        }
        else{ assert(0);}
    ...
    if(klass != NULL) {
#ifdef ORDER_DEBUG
        printf("[ORDER]: In BootstrapClassLoader::DoLoadClass ==> Class : %s is Loaded when thread[%d] try to load it!!\n", className->bytes, hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("in do load class: ClassName ==> %s, thread [%d]. \n", className->bytes, hythread_self()->thread_id);
#endif
    ASSERT_RAISE_AREA;
    ...
    if(vm_order_record){
        klass = StartLoadingClass(env, className);
        ...
        fprintf(order_system_call[tid], "[%d]", 13);
...
//            klass = StartLoadingClass(env, className);
    ...
    if(klass != NULL) {
#ifdef ORDER_DEBUG
        printf("[ORDER]: In UserDefinedClassLoader::DoLoadClass ==> Class : %s is Loaded when thread[%d] try to load it!!\n", className->bytes, hythread_self()->thread_id);
#endif
        return klass;
    }
    if(exn_raised()) {
    ...
    else
    {
        if(jstr_created == 1)
        {
            ...
            while(jstr == NULL)
            {
                ...
                {
#ifdef ORDER_DEBUG
                    printf("usleep in classloader.cpp, %s\n", class_name_with_dots->bytes);
#endif
                    usleep(1000);
                    hythread_yield();
                    hythread_safe_point();
...
#ifdef ORDER_DEBUG
        fprintf(order_system_call[tid], "[%d] ", 2);
#endif
        fprintf(order_system_call[tid], "%d\n", jstr_created);
    }
    else
...
#ifdef ORDER_DEBUG
    printf("Enter vm_execute_java_method_array, thread [%d], %s, %s\n", hythread_self()->thread_id, (char *)className->bytes, (char *)method->get_name()->bytes);
#endif
    vm_execute_java_method_array((jmethodID) method, &res, args);
#ifdef ORDER_DEBUG
    printf("Exit vm_execute_java_method_array, thread [%d], %s, %s\n", hythread_self()->thread_id, (char *)className->bytes, (char *)method->get_name()->bytes);
#endif
...
//debug used
    ManagedObject *class_obj = *(clss->get_class_handle());
    assert(class_obj != NULL);
    printf("[CLASS_OBJ_MAPPING]: Class name : %s  ==> Object[%d, %d]\n", className->bytes, class_obj->alloc_tid, class_obj->alloc_count);
//end
    tmn_suspend_enable();
