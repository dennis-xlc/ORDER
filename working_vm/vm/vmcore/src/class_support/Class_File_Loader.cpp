...
AttributeID code_attrs[] = {
	...
	{"ThreadLocalAttribute", NULL, ATTR_ThreadLocalTag},
#endif
    {NULL, NULL, ATTR_UNDEF}
};
...
#ifdef THREAD_LOCAL_TAG
bool Method::_parse_thread_locals(unsigned attr_len, ByteReader &cfs) {
    uint16 n_threadlocals;
    if(!cfs.parse_u2_be(&n_threadlocals)) {
        REPORT_FAILED_METHOD("could not parse thread_local_table_length "
            "while parsing Thread_Local_Table attribute");
        return false;
    }
    /*
	...
	*		u4 attribute_length
	*		u2 thread_locals_table_length
	*		u3 attribute[attribute_length/3]
	*	}
    */
    unsigned real_lnt_attr_len = sizeof(n_threadlocals) + n_threadlocals * 3 * sizeof(U_8);
    if(real_lnt_attr_len != attr_len) {
        REPORT_FAILED_METHOD("Thread Local attribute has wrong length  ("
            << attr_len << " vs. " << real_lnt_attr_len << ")" );
        return false;
    }

	assert(!_thread_local_table);
    _thread_local_table =
        (Thread_Local_Table *)STD_MALLOC(sizeof(Thread_Local_Table) +
        sizeof(Thread_Local_Table) * (n_threadlocals - 1));
    // ppervov: FIXME: should throw OOME
    _thread_local_table->length = n_threadlocals;

    for (unsigned j = 0; j < n_threadlocals; j++) {
        uint16 start_pc;
        if(!cfs.parse_u2_be(&start_pc)) {
            REPORT_FAILED_METHOD("could not parse start_pc "
                "while parsing Thread_Local_Table");
            return false;
        }

		U_8 padding;
		if(!cfs.parse_u1(&padding)) {
            REPORT_FAILED_METHOD("could not parse padding "
                "while parsing Thread_Local_Table");
            return false;
        }
		assert(padding == 0);
        ...
        if(start_pc >= _byte_code_length) {
            REPORT_FAILED_METHOD("start_pc in Thread_Local_Table "
                "points outside the code");
            return false;
        }
		
        _thread_local_table->table[j].start_pc = start_pc;
    }
    return true;
} //Method::_parse_thread_local
#endif
                         ...
                         ByteReader& cfs)
{
    ...
    for (i=0; i<n_attrs; i++) {
        ...
        case ATTR_StackMapTable:
            ...
            break;
#ifdef THREAD_LOCAL_TAG
		case ATTR_ThreadLocalTag:{
			numThreadLocals++;
            if (numThreadLocals > 1) {
                REPORT_FAILED_METHOD(" there is more than one ThreadLocal attribute");
				assert(false);
                return false;
            }

			if (!_parse_thread_locals(attr_len, cfs)) {
				return false;
			}
			break;
		}
...
#ifdef RECORD_STATIC_IN_FIELD
        m_fields[i].alloc_count = m_hash_code;
        m_fields[i].alloc_tid = (U_16)(i+1);
        m_fields[i].access_tid = (U_32)pthread_self();
#endif
