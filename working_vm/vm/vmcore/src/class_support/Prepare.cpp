...
FILE *order_system_call_prepare[ORDER_THREAD_NUM];

POINTER_SIZE_INT prefetch_hashcode[ORDER_THREAD_NUM];
...
bool Class::prepare(Global_Env* env)
{
    ...
    if(!vm_order_record){
        ...
        if(order_system_call_prepare[tid] == NULL){
            char name[40];
            sprintf(name, "SYSTEM_CALL_PREPARE.%d.log", tid);

            order_system_call_prepare[tid] = fopen64(name, "r");
            if(order_system_call_prepare[tid] == NULL){
                goto wait_for_prepare;
            }

#ifdef ORDER_DEBUG
            if (order_system_call_prepare[tid] != NULL)
                fseek(order_system_call_prepare[tid], 0, SEEK_SET);

            assert(order_system_call_prepare[tid] != NULL);
            assert(ferror(order_system_call_prepare[tid]) == 0);
            assert(order_system_call_prepare[tid]->_IO_read_ptr != NULL);
#endif

        }

        if(0 == (U_32)prefetch_hashcode[tid]){
            if(!feof(order_system_call_prepare[tid])){
//                fread(&prefetch_hashcode[tid], sizeof(prefetch_hashcode[tid]), 1, order_system_call_prepare[tid]);
                fscanf(order_system_call_prepare[tid], "%d ", &prefetch_hashcode[tid]);
            }
        }

        if((U_32)m_hash_code != (U_32)prefetch_hashcode[tid]){
			
wait_for_prepare:
            while(!(is_at_least_prepared() || in_error())){
#ifdef ORDER_DEBUG
                printf("Thread[%d]: usleep in prepare!!!\n", hythread_self()->thread_id);
#endif
                usleep(1000);
            }
            return true;
...
#ifdef ORDER_DEBUG
    if(!vm_order_record){
        assert(!(is_at_least_prepared() || in_error()));
    }
#endif
    ...
    if(vm_order_record){
        ...
        if(order_system_call_prepare[tid] == NULL){
            ...
            order_system_call_prepare[tid] = fopen64(name, "a+");
#ifdef ORDER_DEBUG
            assert(order_system_call_prepare[tid]);
#endif
        }

        fprintf(order_system_call_prepare[tid], "%d \n", m_hash_code);
    ...
    if(m_name == env->JavaLangClass_String) {
        ...
        printf("env->vm_class_offset : 0x%x\n", env->vm_class_offset);
