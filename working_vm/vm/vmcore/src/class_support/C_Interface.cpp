...
#ifdef ORDER_DEBUG
//    printf("[STRING]: (%d, %s, %d)\n", hythread_self()->thread_id, str->bytes, (long)str->intern.raw_ref);
#endif
...
#include <stdio.h>
FILE *order_system_call_in_compile[ORDER_THREAD_NUM];
#include "open/hythread_ext.h"
#include "open/hythread.h"


U_32 order_thread_lazy_resolution[ORDER_THREAD_NUM] = { 0 };
bool is_lazy_resolution_enable()
{
   U_32 tid = hythread_self()->thread_id;
#ifdef ORDER_DEBUG
    assert(tid < ORDER_THREAD_NUM);
    printf("[IS_LAZY_RESOLUTION_ENABLE]: Thread %d TEST value %d\n", tid, order_thread_lazy_resolution[tid]);
#endif
    return order_thread_lazy_resolution[tid] >= 1;
}

bool is_show_parse_process()
{
   U_32 tid = hythread_self()->thread_id;
#ifdef ORDER_DEBUG
    assert(tid < ORDER_THREAD_NUM);
#endif
    return order_thread_lazy_resolution[tid] == 2;
}


void record_System_Call_In_Compile(U_32 constPoolIndex, bool isUnresolved)
{
    U_32 tid = hythread_self()->thread_id;
    if (order_system_call_in_compile[tid] == NULL)
    {
        char name[40];
        sprintf(name, "SYSTEM_CALL_IN_COMPILE.%d.log", tid);
        order_system_call_in_compile[tid] = fopen64(name, "a+");
    }
	
#ifdef ORDER_DEBUG
    assert(order_system_call_in_compile[tid] != NULL);

    fprintf(order_system_call_in_compile[tid], "[%d] ", 35);
		
    fprintf(order_system_call_in_compile[tid], "%d ", constPoolIndex);

#endif
        fprintf(order_system_call_in_compile[tid], "%d\n", isUnresolved ? 1 : 0);
}

bool get_System_Call_In_Compile(U_32 constPoolIndex)
{
    U_32 tid = hythread_self()->thread_id;
    if (order_system_call_in_compile[tid] == NULL)
    {
        ...
        order_system_call_in_compile[tid] = fopen64(name, "r");
    }
#ifdef ORDER_DEBUG
    assert(order_system_call_in_compile[tid] != NULL);

    U_32 bit_num = 0;
    fscanf(order_system_call_in_compile[tid], "[%d] ", &bit_num);
    assert(bit_num == 35);
		
    U_32 index_temp = 0;
    fscanf(order_system_call_in_compile[tid], "%d ", &index_temp);
    assert(index_temp == constPoolIndex);

#endif
    U_32 isUnresolved = 0;
    fscanf(order_system_call_in_compile[tid], "%d\n", &isUnresolved);
    return (isUnresolved == 1);

}

bool is_field_final(Class_Handle cl, U_16 index)
{
    Class_Member* cur_field = ((Class *)cl)->get_constant_pool().get_ref_class_member(index);

    assert(cur_field);
    return cur_field->is_final();
}

//yzm
//interface to get thread id in OPT
bool is_initializing() {
    return vm_is_initializing;
}

void* get_tm_self_tls_addr() {
    return (void*)hythread_self;
}

unsigned get_tid_offset() {
    return (unsigned)(POINTER_SIZE_INT)(&((HyThread*)NULL)->thread_id);
}

void* vm_enter_method(){
    return (void*)gc_enter_method;
}

void* vm_method_return(){
    return (void*)gc_method_return;
}

extern bool vm_order_record;

bool vm_is_record(){
    return vm_order_record;
}

/*
void* vm_enter_sleep(){
    return (void*)gc_enter_sleep;
}
*/

void* get_record_access_info_method(){
    return (void*)gc_record_info;
}

void* get_replay_access_info_method(){
    return (void*)gc_get_interleaving_info;
}
...
#ifdef ORDER
ManagedObject *vm_get_managed_null_object()
{
    return (ManagedObject*)VM_Global_State::loader_env->managed_null;
} //vm_get_managed_null_object
...
typedef struct Field_Access_Info {
    U_32 access_count;
    U_32 alloc_count;
    U_32 access_tid;
    U_16 alloc_tid;
...
typedef struct Field_Access_Pool{
    Field_Access_Info *pool;
    POINTER_SIZE_INT used_num;
    POINTER_SIZE_INT fill_num;
    SpinLock lock;
} Field_Access_Pool;

enum POOL_SWAP_CAUSE{
  NIL,
  POOL_DESTRUCT,
  POOL_IS_FULL
};

#define FIELD_ACCESS_INFO_NUM (1<<20) // the number of access info in every local access pool
#define FIELD_MAX_POOL_NUM 10
#define FIELD_ACCESS_INFO_SIZE  sizeof(Field_Access_Info)


bool binary_mode = TRUE;
//bool binary_mode = FALSE;
unsigned int pool_swap_count = 0;
FILE *fieldAccess_logFile = NULL;

/**************** Single Pool ************************/
static Field_Access_Pool  global_field_access_pool;
Field_Access_Pool *get_Global_Field_Access_Pool() {
	return &global_field_access_pool;
}


void *alloc_field_access_pool()
{
    unsigned int access_pool_size = FIELD_ACCESS_INFO_SIZE * FIELD_ACCESS_INFO_NUM;
    void * access_pool = STD_MALLOC(access_pool_size);
#ifdef ORDER_DEBUG
    if(access_pool == NULL){
        fprintf(stderr,"[xlc]: Could NOT malloc local access pool in alloc_field_access_pool!\n");
        assert(0);
    }
#endif
...
void reset_field_access_pool(Field_Access_Info *access_pool)
{
    memset(access_pool, 0, FIELD_ACCESS_INFO_SIZE * FIELD_ACCESS_INFO_NUM);
}

void free_field_access_pool(Field_Access_Info *access_pool)
{
#ifdef ORDER_DEBUG
...
void init_global_field_access_pool()
{
#ifdef ORDER_DEBUG
    fprintf(stderr, "[xlc]: GC init global field access pool\n");
#endif
    Field_Access_Pool *pool = get_Global_Field_Access_Pool();
    pool->pool = (Field_Access_Info *)alloc_field_access_pool();
    pool->used_num = 0;
    pool->lock = FREE_LOCK;

    fieldAccess_logFile = fopen64("ORDER_FIELD_ACCESS_RECORD.log", "w+");
#ifdef ORDER_DEBUG
    if(fieldAccess_logFile == NULL){
        fprintf(stderr, "[xlc]: could NOT open file ORDER_FIELD_ACCESS_RECORD.log\n");
        assert(0);
    }
#endif
}

void swap_field_access_pool_info(Field_Access_Pool *pool, unsigned int swap_cause)
{
#ifdef ORDER_DEBUG
    assert(pool != NULL);
    assert(fieldAccess_logFile != NULL);

    pool_swap_count++;
	
    if(swap_cause == POOL_IS_FULL){
        assert(pool->used_num == FIELD_ACCESS_INFO_NUM);
    ...
    if(binary_mode){
#endif
        unsigned int ret = fwrite((char *)pool->pool, FIELD_ACCESS_INFO_SIZE, pool->used_num, fieldAccess_logFile);
    
#ifdef ORDER_DEBUG
        assert(ret <= FIELD_ACCESS_INFO_SIZE * pool->used_num);
    }
    else{
//        fprintf(logFile, "[%d]: Swap access pool info\n",swap_count);
        Field_Access_Info *access_pool = pool->pool;
        for(unsigned int i = 0; i < pool->used_num; i++){
            Field_Access_Info *record = &access_pool[i];

		fprintf(fieldAccess_logFile, "AllocTID: %d,  AllocCount: %d,  AccessTID: %d,  AccessCount: %d,  RWbit: %d\n", 
...
void record_field_access_info(Field_Handle field)
{
    ...
    assert(pool->used_num < FIELD_ACCESS_INFO_NUM);
#endif
    Field_Access_Info *record_info = &access_pool[pool->used_num];
#ifdef ORDER_DEBUG
    assert(field->alloc_count != 0);
    assert(field->alloc_tid != 0);
#endif

    //these assignment may be replaced by memcpy operation
    record_info->access_count = field->access_count;
    record_info->access_tid = field->access_tid;
    record_info->alloc_count = field->alloc_count;
    record_info->alloc_tid = field->alloc_tid;

#ifdef REPLAY_OPTIMIZE
	record_info->rw_bit = ((field->order_padding & OBJ_RW_BIT) == 0) ? 0 : 1;
	field->order_padding &= OBJ_CLEAN_RW_BIT;
#endif

    pool->used_num++;

    if(pool->used_num == FIELD_ACCESS_INFO_NUM){
        swap_field_access_pool_info(pool, POOL_IS_FULL);
        reset_field_access_pool(pool->pool);
        pool->used_num = 0;
    }
    unlock(pool->lock);
}
...
void destruct_global_field_access_pool()
{
#ifdef ORDER_DEBUG
    printf("[xlc]: GC destruct global access pool\n");
#endif
    Field_Access_Pool *pool = get_Global_Field_Access_Pool();
    swap_field_access_pool_info(pool, POOL_DESTRUCT);
    fflush(fieldAccess_logFile);
    fclose(fieldAccess_logFile);
#ifdef ORDER_DEBUG
    assert(pool->pool != NULL);
#endif
    free_field_access_pool(pool->pool);
    pool->pool = NULL;
    pool->used_num = 0;
    pool->lock = FREE_LOCK;
...
void vm_record_field_access_info(Field_Handle field)
{
//    printf("[GC_RECORD_INFO]: Recording field (%s) ...\n", field->get_name()->bytes);
    record_field_access_info(field);
}


void* get_record_field_access_info_method(){
    return (void*)vm_record_field_access_info;
}

#include "GlobalClassLoaderIterator.h"

Class **get_all_loaded_classes(int *class_num)
{
    Class **klass;
    Class **classes;
    unsigned int index;
     ...
     */
    unsigned int class_array_size = sizeof(Class *) * count;
    classes = (Class **)STD_MALLOC(class_array_size);
#ifdef ORDER_DEBUG
    if(classes == NULL){
        fprintf(stderr,"[xlc]: Could NOT malloc class_array in get_all_loaded_classes!\n");
        assert(0);
...
#endif
    memset(classes, 0, class_array_size);
...
void finalize_all_field_access_info()
{

    Class_Handle klass;
    int all_classes_num = 0;
    Class **all_loaded_classes = get_all_loaded_classes(&all_classes_num);
    if(all_loaded_classes == NULL || all_classes_num == 0){
        printf("[ERROR]: In finalize_all_field_access_info, fail to get_all_loaded_classes !!!! \n");
        assert(0);
    }

#ifdef ORDER_DEBUG
    printf("[LOADED CLASS]: Totally %d classes are loaded !!!\n",all_classes_num);
#endif
    
    U_16 num_of_field;
    for(int i = 0; i < all_classes_num; i++){
        klass = (Class_Handle)all_loaded_classes[i];
#ifdef ORDER_DEBUG
        if(klass == NULL){
            printf("[ERROR]: In finalize_all_field_access_info, klass == NULL !!!! \n");
            assert(0);
        }
        printf("[LOADED CLASS]: Class name is %s \n", klass->get_name()->bytes);
#endif
        num_of_field = class_number_fields(klass);

#ifdef ORDER_DEBUG
    printf("[LOADED CLASS]: Class (%s) has %d fields !!!\n", klass->get_name()->bytes, num_of_field);
#endif

        Field_Handle field_tmp;

        for(U_16 j = 0; j < num_of_field; j++){
            field_tmp = (Field *)class_get_field(klass, j);
            if(field_tmp != NULL){
#ifdef ORDER_DEBUG
                printf("[LOADED CLASS]: Field name is %s !!!\n", field_tmp->get_name()->bytes);
#endif
                if(field_tmp->access_tid != 0 && field_tmp->access_count != 0){
                    vm_record_field_access_info(field_tmp);
                }
            }
            else{ assert(0); }
        }
    ...
    destruct_global_field_access_pool();
}
