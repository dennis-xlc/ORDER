...
{
#ifdef ORDER_DEBUG
    printf("start : class_find_loaded, tid : %d\n", hythread_self()->thread_id);
#endif
...
#ifdef ORDER_DEBUG
    printf("end : class_find_loaded, tid : %d\n", hythread_self()->thread_id);
#endif
...
Class_Handle class_find_class_from_loader(Class_Loader_Handle loader, const char* n, Boolean init)
{
...
#ifdef ORDER_DEBUG
    printf("start : class_find_class_from_loader, tid : %d\n", hythread_self()->thread_id);
#endif
    ASSERT_RAISE_AREA;
    ...
    {
#ifdef ORDER_DEBUG
        printf("end : class_find_class_from_loader, tid : %d\n", hythread_self()->thread_id);
#endif
        return NULL;
    }
...
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_ClassLoader_defineClass0, tid : %d\n", hythread_self()->thread_id);
#endif
...
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_ClassLoader_defineClass0, tid : %d, classname : %s\n", hythread_self()->thread_id, clssname);
#endif
...
#ifdef ORDER_DEBUG
    printf("end : Java_java_lang_ClassLoader_defineClass0, tid : %d, classname : %s\n", hythread_self()->thread_id, clssname);    
#endif
    if(clssname)
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_loadBootstrapClass, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_ClassLoader_findLoadedClass, tid : %d\n", hythread_self()->thread_id);
#endif

    if (NULL == name) {
#ifdef ORDER_DEBUG
        printf("class name : NULL\n");
#endif
        return NULL;
    } 
...
#ifdef ORDER_DEBUG
        fprintf(order_system_call[tid], "[%d] ", 3);
        fprintf(order_system_call[tid], "%s ", buf);
...
//        fflush(order_system_call[tid]);
    ...
    else
    {
        if (class_exist == 1)
        {
            while ((clss = class_find_loaded(loader, buf)) == NULL)
            {
                usleep(1000);
#ifdef ORDER_DEBUG
                printf("usleep : java_lang_VMClassResitry.cpp : wait class_find_loaded returns not NULL.\n");
#endif
            }
        }
        else if (class_exist == 0)
        {
            clss = NULL;
        }
        else
...
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_ClassLoader_registerInitiatedClass, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getClassNative, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getClassLoader0, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getComponentType, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getDeclaredClasses, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getDeclaredConstructors, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getDeclaredFields, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getDeclaredMethods, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getDeclaringClass, tid : %d\n", hythread_self()->thread_id);
#endif
    Class_Handle clss = jni_get_class_handle(jenv, clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getInterfaces, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getModifiers, tid : %d\n", hythread_self()->thread_id);
#endif
    Class_Handle clss = jni_get_class_handle(jenv, clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getName, tid : %d\n", hythread_self()->thread_id);
#endif
    ASSERT_RAISE_AREA;
    Class* clss = jclass_to_struct_Class(clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getSuperclass, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getSystemPackages, tid : %d\n", hythread_self()->thread_id);
#endif
#ifdef ORDER
//    hythread_self()->isInVMRegistry = 1;
/*
    {
        ...
        for (Package_Table::const_iterator it = ptab->begin(), end = ptab->end(); 
            it != end; ++it)
        {
            const char* name = (*it).first->bytes;
            printf("package name : %s, jar : %s\n", name, (*it).second->get_jar());
...
#ifdef ORDER_DEBUG
        printf("end : Java_java_lang_VMClassRegistry_getSystemPackages, tid : %d\n", hythread_self()->thread_id);
#endif
#ifdef ORDER
//        hythread_self()->isInVMRegistry = 0;
#endif
...
#ifdef ORDER_DEBUG
        fprintf(order_system_call[tid], "[%d] ", 1);
#endif
        fprintf(order_system_call[tid], "%d\n", buf_len);
//        fflush(order_system_call[tid]);
//        printf("buf_len : %d\n", buf_len);
    ...
    if (result) 
    {
        ...
        if(vm_order_record){
	       ...
	       assert(0);
            ...
            fprintf(order_system_call[tid], "[%d] ", 16);
#endif
            fprintf(order_system_call[tid], "%d\n", p_num);
//            fflush(order_system_call[tid]);
//            printf("p_num : %d\n", p_num);
        ...
        for (index = 0; index < p_num; )
        {
            ...
            if(vm_order_record){
               U_32 tid = hythread_self()->thread_id;
                ...
                fprintf(order_system_call[tid], "[%d] ", 15);
#endif
		if(strcmp(name, "") == 0){
                    fprintf(order_system_call[tid], "%s\n", null_package_name);
		}
		else{
                    fprintf(order_system_call[tid], "%s\n", name);
		}
...
//                printf("name : %s\n", name);
            ...
            else
            {
                ...
                fscanf(order_system_call[tid], "%s\n", buf);
                if(strcmp(buf, null_package_name) == 0){
                    name = "";
                }
                else{
                    name = buf;
...
//                printf("jar : %s\n", jar);
            ...
            else
            {
                ...
                Package_Table::const_iterator it = ptab->begin(), end = ptab->end();
                for (; it != end; ++it)
                {
                    if (strcmp(name, (*it).first->bytes) == 0)
                    {
                        jar = (*it).second->get_jar();
                        break;
                    }
                }
#ifdef ORDER_DEBUG
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_initializeClass, tid : %d\n", hythread_self()->thread_id);
#endif
    ASSERT_RAISE_AREA;
    assert(clazz != NULL);
    Class *clss = jni_get_class_handle(jenv, clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_isArray, tid : %d\n", hythread_self()->thread_id);
#endif
    Class_Handle ch = jni_get_class_handle(jenv, clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_isAssignableFrom, tid : %d\n", hythread_self()->thread_id);
#endif
    // check parameters
    if (!clazz)
    {
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_isInstance, tid : %d\n", hythread_self()->thread_id);
#endif
    // null object
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_isPrimitive, tid : %d\n", hythread_self()->thread_id);
#endif
    Class_Handle ch = jni_get_class_handle(jenv, clazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_loadArray, tid : %d\n", hythread_self()->thread_id);
#endif
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_loadLibrary, tid : %d\n", hythread_self()->thread_id);
#endif
    // check filename
    if (! filename)
    {
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getEnclosingClass, tid : %d\n", hythread_self()->thread_id);
#endif
    assert(jclazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getEnclosingMember, tid : %d\n", hythread_self()->thread_id);
#endif
    assert(jclazz);
    Class* clazz = jclass_to_struct_Class(jclazz);
...
{
#ifdef ORDER_DEBUG
    printf("start : Java_java_lang_VMClassRegistry_getSimpleName, tid : %d\n", hythread_self()->thread_id);
#endif
    ASSERT_RAISE_AREA;
    assert(jclazz);
    Class* clazz = jclass_to_struct_Class(jclazz);
