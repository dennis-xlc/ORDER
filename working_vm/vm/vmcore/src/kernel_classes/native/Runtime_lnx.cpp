...
#ifdef ORDER
FILE* system_call_input[ORDER_THREAD_NUM];
FILE* system_call_input_content[ORDER_THREAD_NUM];
...
jint JNICALL Java_java_lang_Runtime_00024SubProcess_00024SubInputStream_readInputByte0 (JNIEnv *env, jobject obj, jlong inputHandle) {
    ...
    if (vm_order_record)
    {
        res = read((int)inputHandle, ca, (unsigned) 1);
        if (system_call_input[tid] == NULL)
        {
            char name[40];
            sprintf(name, "SYSTEM_CALL_INPUT.%d.log", tid);

            system_call_input[tid] = fopen64(name, "a+");
#ifdef ORDER_DEBUG
            assert(system_call_input[tid]);
#endif
        }

#ifdef ORDER_DEBUG
        int bit_num = 10;
        fwrite(&bit_num, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);
#endif

        fwrite(&res, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);

        if (res == 1)
        {
            if (system_call_input_content[tid] == NULL)
            {
                char name[40];
                sprintf(name, "SYSTEM_CALL_INPUT_CONTENT.%d.log", tid);

                system_call_input_content[tid] = fopen64(name, "a+");
#ifdef ORDER_DEBUG
                assert(system_call_input_content[tid]);
#endif
            }

            fwrite(&ca, sizeof(char), 1, system_call_input_content[tid]);
//            fflush(system_call_input_content[tid]);
        }
    }
    else
    {
        ...
        if (system_call_input[tid] == NULL)
        {
            ...
            system_call_input[tid] = fopen64(name, "r");
#ifdef ORDER_DEBUG
            assert(system_call_input[tid]);
#endif
        }
...
#ifdef ORDER_DEBUG
        int bit_num = 0;
        fread(&bit_num, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);
        assert(bit_num == 10);
#endif

        fread(&temp_res, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);

//        printf("temp_res got : %d\n", temp_res);
        ...
        if (temp_res == 1)
        {
            if (system_call_input_content[tid] == NULL)
            {
                ...
                system_call_input_content[tid] = fopen64(name, "r");
#ifdef ORDER_DEBUG
                assert(system_call_input_content[tid]);
#endif
            }

            fread(&ca, sizeof(char), 1, system_call_input_content[tid]);
//            fflush(system_call_input_content[tid]);

//            printf("character read: %c\n", ((unsigned char) ca[0])&0xFF);
/*
            char ttt = 0,rrr = 0;
            rrr = read((int)inputHandle, &ttt, (unsigned) 1);
            printf("character read_ori: %c\n", ((unsigned char) ttt)&0xFF);
*/
            res = 1;
        }
        else
...
jint JNICALL Java_java_lang_Runtime_00024SubProcess_00024SubInputStream_available0 (JNIEnv *env, jobject obj, jlong inputHandle) {
    ...
    if (vm_order_record)
    {
        int ioctl_res = ioctl((int) inputHandle, FIONREAD, &res);
...
//        fflush(system_call_input[tid]);
        ...
        fwrite(&ioctl_res, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);
...
//        fflush(system_call_input[tid]);
        ...
        fread(&temp_ioctl_res, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);

        fread(&temp_res, sizeof(int), 1, system_call_input[tid]);
//        fflush(system_call_input[tid]);

//        printf("[ioctl] temp_ioctl_res got : %d\n", temp_ioctl_res);
//        printf("[ioctl] temp_res got : %d\n", temp_res);
/*
        if (temp_ioctl_res != -1 && temp_res > 0)
        {
            ioctl_res = ioctl((int) inputHandle, FIONREAD, &res);
            while (ioctl_res == -1 || res <= 0)
            {
                usleep(100);
                printf("usleep in runtime_lnx.cpp\n");
                ioctl_res = ioctl((int) inputHandle, FIONREAD, &res);
            }
            res = temp_res;
        }
...
//                printf("[available] temp_res got : %d\n", temp_res);
