...
NativeCodePtr compile_create_lil_jni_stub(Method_Handle method, void* func, NativeStubOverride nso)
{
    ...
    if (is_synchronised) {
#ifdef ORDER
        bool bypass = false;//set true if is_synchronised and we need NOT to record the access
#endif
        if (is_static) {
#ifdef ORDER
            if(bypass){
                cs = lil_parse_onto_end(cs,
                                    ...
                                    lil_npc_to_fp(vm_helper_get_addr(VM_RT_MONITOR_ENTER_PURE)));
            }
            else
#endif
            cs = lil_parse_onto_end(cs,
...
#ifdef ORDER
int is_in_do_jit_recompiled_method_callbacks[ORDER_THREAD_NUM] = { 0 };
int is_in_compile_method_with_params[ORDER_THREAD_NUM] = { 0 };
#endif
JIT_Result compile_do_compilation_jit(Method* method, JIT* jit)
{
#ifdef ORDER_DEBUG
    U_32 tid1 = hythread_self()->thread_id;
    is_in_do_jit_recompiled_method_callbacks[tid1] = is_in_do_jit_recompiled_method_callbacks[tid1] + 1;
#endif
...
#ifdef ORDER_DEBUG
    U_32 tid2 = hythread_self()->thread_id;
    is_in_do_jit_recompiled_method_callbacks[tid2] = is_in_do_jit_recompiled_method_callbacks[tid2] - 1;
#endif
            return JIT_SUCCESS;
        }
    }
...
#ifdef ORDER_DEBUG
    is_in_compile_method_with_params[tid1] = is_in_compile_method_with_params[tid1] + 1;
#endif
...
int is_in_compile_prepare_native_method[ORDER_THREAD_NUM] = { 0 };
int is_in_CompileMethod[ORDER_THREAD_NUM] = { 0 };
...
#ifdef ORDER_DEBUG
        U_32 tid1 = hythread_self()->thread_id;
        is_in_compile_prepare_native_method[tid1] = is_in_compile_prepare_native_method[tid1] + 1;
#endif
...
#ifdef ORDER_DEBUG
        U_32 tid2 = hythread_self()->thread_id;
        is_in_CompileMethod[tid2] = is_in_CompileMethod[tid2] + 1;
#endif
        JIT_Result res = VM_Global_State::loader_env->em_interface->CompileMethod(method);
#ifdef ORDER_DEBUG
        is_in_CompileMethod[tid2] = is_in_CompileMethod[tid2] - 1;
#endif
        return res;
...
#ifdef ORDER
char* method_tobe_compile[ORDER_THREAD_NUM];
char* class_of_method_tobe_compile[ORDER_THREAD_NUM];
char* sig_of_method_tobe_compile[ORDER_THREAD_NUM];
FILE *systemCallCompile[ORDER_THREAD_NUM];
extern bool vm_order_record;
extern bool vm_is_initializing;


extern U_32 order_thread_lazy_resolution[ORDER_THREAD_NUM];
void set_lazy_resolution(U_32 value)
{
    U_32 tid = hythread_self()->thread_id;
#ifdef ORDER_DEBUG
    assert(tid < ORDER_THREAD_NUM);
    printf("[SET_LAZY_RESOLUTION]: Thread %d set value %d\n", tid, value);
#endif
    order_thread_lazy_resolution[tid] = value;
}


int is_in_compile_me[ORDER_THREAD_NUM] = { 0 };
#endif
...
#ifdef ORDER_DEBUG
    printf("enter compile_me : tid[%d]\n", hythread_self()->thread_id);
#endif

#ifdef ORDER
get_lock:
    method->lock();
    if (method->get_state() == Method::ST_Compiling && method->compiling_thread != hythread_self()->thread_id)
    {
        method->unlock();
        while(method->get_state() == Method::ST_Compiling)
        {
#ifdef ORDER_DEBUG
            printf("usleep in compile.cpp, tid : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
...
//#endif
        if(method->get_state() == Method::ST_Compiled){		
            void * method_code_addr = method->get_code_addr();
            method->unlock();
            return method_code_addr;
        }
        else if(method->get_state() == Method::ST_NotCompiled || method->get_state() == Method::ST_Compiling){
            method->unlock();
            goto get_lock;
        }

//        return method->get_code_addr();
    }
    else if(method->get_state() == Method::ST_Compiled){
        method->unlock();
        return method->get_code_addr();
...
//    printf("set compile state in compile.cpp, tid : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
//		method->get_class()->get_name()->bytes);

#ifdef ORDER_DEBUG
    if(method->get_state() == Method::ST_Compiling){
        assert(method->compiling_thread == hythread_self()->thread_id);
    }
    else{
        assert(method->get_state() == Method::ST_NotCompiled);
    }
#endif
    method->set_state(Method::ST_Compiling);
    method->compiling_thread = hythread_self()->thread_id;
    method->unlock();

    U_32 tid = hythread_self()->thread_id;

    if(vm_order_record){
        if (systemCallCompile[tid] == NULL)
        {
            char name[40];
            sprintf(name, "SYSTEM_CALL_COMPILE.%d.log", tid);

            systemCallCompile[tid] = fopen64(name, "a+");
        }
    }
    else
    {
        if (systemCallCompile[tid] == NULL)
        {
            ...
            systemCallCompile[tid] = fopen64(name, "r");
        }
    }

    String* method_name = method->get_name();
//    String* method_signature = method->get_method_sig()->sig;
//    String* method_signature = method->get_signature();
    String* method_signature = method->get_descriptor();
    assert(method_signature != NULL);
    const String* class_name = method->get_class()->get_name();

    if(vm_order_record){

        U_32 method_name_len = method_name->len;
        fwrite(&method_name_len, sizeof(U_32), 1, systemCallCompile[tid]);
        fwrite(method_name->bytes, sizeof(char), method_name_len, systemCallCompile[tid]);

        U_32 class_name_len = class_name->len;
        fwrite(&class_name_len, sizeof(U_32), 1, systemCallCompile[tid]);
        fwrite(class_name->bytes, sizeof(char), class_name_len, systemCallCompile[tid]);


        U_32 method_signature_len = method_signature->len;
        fwrite(&method_signature_len, sizeof(U_32), 1, systemCallCompile[tid]);
        fwrite(method_signature->bytes, sizeof(char), method_signature_len, systemCallCompile[tid]);
		
//        printf("compile method: %s, %s, %d\n", method_name->bytes, class_name->bytes, tid);
    }
    else
    {
        if (method_tobe_compile[tid] == NULL && class_of_method_tobe_compile[tid] == NULL)
        {
            if (systemCallCompile[tid] == NULL)
            {
                method->lock();
                method->set_state(Method::ST_NotCompiled);
                method->compiling_thread = 0;
                method->unlock();
                usleep(1000);
#ifdef ORDER_DEBUG
                printf("usleep in compile.cpp, this thread do not compile method : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
        	      method->get_class()->get_name()->bytes);
#endif
                hythread_yield();
                hythread_safe_point();
            ...
            {
                U_32 method_name_len;
                uint16 index_temp = 0;

                fread(&method_name_len, sizeof(U_32), 1, systemCallCompile[tid]);

                if (feof(systemCallCompile[tid]))
                {
                    method->lock();
                    method->set_state(Method::ST_NotCompiled);
                    method->compiling_thread = 0;
                    ...
                    usleep(1000);
#ifdef ORDER_DEBUG
                    printf("usleep in compile.cpp, [eof]not method tobe compiled : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
                        method->get_class()->get_name()->bytes);
#endif
                    hythread_yield();
                    hythread_safe_point();
                ...
                method_tobe_compile[tid] = (char*)malloc(sizeof(char) * (method_name_len + 1));
                fread(method_tobe_compile[tid], sizeof(char), method_name_len, systemCallCompile[tid]);
                method_tobe_compile[tid][method_name_len] = '\0';
//                fflush(systemCallCompile[tid]);
#ifdef ORDER_DEBUG
                assert(method_tobe_compile[tid] != NULL);
#endif
            }

            {
                ...
                fread(&class_name_len, sizeof(U_32), 1, systemCallCompile[tid]);
                ...
                class_of_method_tobe_compile[tid] = (char*)malloc(sizeof(char) * (class_name_len + 1));
                fread(class_of_method_tobe_compile[tid], sizeof(char), class_name_len, systemCallCompile[tid]);
                class_of_method_tobe_compile[tid][class_name_len] = '\0';
//                fflush(systemCallCompile[tid]);
#ifdef ORDER_DEBUG
                assert(class_of_method_tobe_compile[tid] != NULL);
#endif
            }

            {
                U_32 method_signature_len;

                fread(&method_signature_len, sizeof(U_32), 1, systemCallCompile[tid]);
                ...
                sig_of_method_tobe_compile[tid] = (char*)malloc(sizeof(char) * (method_signature_len + 1));
                fread(sig_of_method_tobe_compile[tid], sizeof(char), method_signature_len, systemCallCompile[tid]);
                sig_of_method_tobe_compile[tid][method_signature_len] = '\0';
//                fflush(systemCallCompile[tid]);
#ifdef ORDER_DEBUG
                assert(sig_of_method_tobe_compile[tid] != NULL);
#endif
            }
        ...
        if (strcmp(method_tobe_compile[tid], method_name->bytes) == 0 && 
		strcmp(class_of_method_tobe_compile[tid], class_name->bytes) == 0 &&
		strcmp(sig_of_method_tobe_compile[tid], method_signature->bytes) == 0)
        {
            free(method_tobe_compile[tid]);
            method_tobe_compile[tid] = NULL;

            free(class_of_method_tobe_compile[tid]);
            class_of_method_tobe_compile[tid] = NULL;

            free(sig_of_method_tobe_compile[tid]);
            sig_of_method_tobe_compile[tid] = NULL;
			//printf("In compile.cpp, method tobe compiled : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
		    //	method->get_class()->get_name()->bytes);
        }
        else
        {
            ...
            usleep(1000);
#ifdef ORDER_DEBUG
            printf("usleep in compile.cpp, not method tobe compiled : %d : %s, %s\n", hythread_self()->thread_id, method->get_name()->bytes,
		    	method->get_class()->get_name()->bytes);
			printf("[1:] not method tobe compiled : %s, %s\n", method_name->bytes, class_name->bytes);
			printf("[2:] not method tobe compiled : %s, %s\n", method_tobe_compile[tid], class_of_method_tobe_compile[tid]);
#endif
            hythread_yield();
            hythread_safe_point();
            hythread_exception_safe_point();
...
#ifdef ORDER_DEBUG
    printf("[COMPILE_ME]: Thread %d is compiling method: %s in Class: %s!!!\n", hythread_self()->thread_id, method->get_name()->bytes,
		    method->get_class()->get_name()->bytes);
#endif

    set_lazy_resolution(1);
    if(strcmp("unparkSuccessor", method->get_name()->bytes) == 0 &&
            strcmp("java/util/concurrent/locks/AbstractQueuedSynchronizer", method->get_class()->get_name()->bytes) == 0){
        set_lazy_resolution(2);
    }
#endif // #ifdef ORDER

    if (exn_raised()) {
#ifdef ORDER
        assert(method->get_state() != Method::ST_Compiling);
#endif
        return NULL;
     }
...
#ifdef ORDER_DEBUG
    U_32 tid1 = hythread_self()->thread_id;
    assert(is_in_compile_me[tid1] >= 0);
    is_in_compile_me[tid1] = is_in_compile_me[tid1] + 1;
#endif
...
#ifdef ORDER_DEBUG
    U_32 tid2 = hythread_self()->thread_id;
    is_in_compile_me[tid2] = is_in_compile_me[tid2] - 1;
    assert(is_in_compile_me[tid1] >= 0);
#endif

#ifdef ORDER
//    if(strcmp("<init>", method->get_name()->bytes) == 0 &&
//            strcmp("org/apache/derby/impl/sql/execute/UpdateResultSet", method->get_class()->get_name()->bytes) == 0){
        set_lazy_resolution(0);
//    }
#ifdef ORDER_DEBUG
    assert(method->get_state() != Method::ST_Compiling);
//    printf("method compiled: %s, %s, %d\n", method_name->bytes, class_name->bytes, tid);
#endif
#endif
