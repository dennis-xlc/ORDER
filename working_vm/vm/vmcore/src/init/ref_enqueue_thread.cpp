...
void ref_enqueue_thread_init(JavaVM *java_vm, JNIEnv* jni_env)
{
    ...
    if(!vm_order_record){
	while(((hythread_t)thread)->thread_id == 0)
        {
            usleep(100);
#ifdef ORDER_DEBUG
            printf("Wait to add thread map entry!!!!\n");
#endif
        }
        gc_add_thread_map_entry((hythread_t)thread);
//        gc_set_sleep_flag((hythread_t)thread);
    }
#endif
...
extern Fin_Thread_Info *fin_thread_info;
extern bool vm_order_record;
#endif
...
/*
        ...
        if (!vm_order_record){
            ...
            gc_set_sleep_flag(hythread_self());
            for(unsigned int i = 0; i < fin_thread_info->thread_num; i++){
                gc_set_sleep_flag((hythread_t)fin_thread_info->thread_ids[i]);
            }
        }
#endif
*/
