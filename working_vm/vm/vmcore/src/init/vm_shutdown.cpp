...
#ifdef ORDER_PER_EVA
void dump_method_count()
{
    printf("start dump method count.\n");
//1Dump methods in bootstrap loader
    Global_Env *env = VM_Global_State::loader_env;
    BootstrapClassLoader *bootstraploader = env->bootstrap_class_loader;

    bootstraploader->dump_method_count();
//1Dump methods in nonbootstrap loader

    unsigned loader_num = (ClassLoader::GetClassLoaderNumber());
    //printf("loader_num is %d\n", loader_num);
    for (unsigned int loader_idx = 0 ; loader_idx < loader_num ; loader_idx ++)
    {
        ClassLoader* loader = (ClassLoader::GetClassLoaderTable())[loader_idx];
        loader->dump_method_count();
    }
}
#endif
...
static IDATA vm_interrupt_entry_point(void * data) {
#ifdef ORDER_DEBUG
	printf("[LY]inside vm_interrupt_entry_point\n");
#endif
	...
	hythread_suspend_all(&iterator, 0);
	hythread_global_lock();
	gc_finalize_order();
#ifdef ORDER_DEBUG
...
#ifdef ORDER // by liyan
	//it seems that there is a bug here, if didnot enable hythread suspend, 
	//some assertion will fail when acquire global lock
	int nSuspendEnable = 0;
	while (!hythread_is_suspend_enabled())
	{	
		hythread_suspend_enable();
		nSuspendEnable++;
	}
	printf("[LY]ctrl+c caught in vm_shutdown.cpp:vm_interrupt_handler\n");
#endif
    int nVMs;
