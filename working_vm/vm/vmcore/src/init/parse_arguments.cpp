...
#ifdef ORDER
        else if (strcmp(option, "-Xorder:replay_mode") == 0) {
            vm_order_record = false;
            hythread_set_IsRecord(false);
            p_env->VmProperties()->set("order.replay_mode", "replay");
        }
        else if(strcmp(option, "-Xorder:record_mode") == 0){
            vm_order_record = true;
            hythread_set_IsRecord(true);
            p_env->VmProperties()->set("order.record_mode", "record");
        }
#endif
        else {
            LECHO(30, "Unknown option {0}" << option);
