...
#ifdef ORDER
Fin_Thread_Info *fin_thread_info = NULL;
#else
...
#ifdef ORDER
        if(!vm_order_record){
            while(((hythread_t)fin_thread_info->thread_ids[i])->thread_id == 0){
	       usleep(100);
#ifdef ORDER_DEBUG
	       printf("Wait to add thread map entry!!!!\n");
#endif
	   }
            gc_add_thread_map_entry((hythread_t)fin_thread_info->thread_ids[i]);  
