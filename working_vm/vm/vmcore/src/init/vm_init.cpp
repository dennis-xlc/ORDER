...
#ifdef ORDER
bool vm_order_record = true;
FILE *order_system_call[ORDER_THREAD_NUM];
FILE *systemCallTimeFile[ORDER_THREAD_NUM];

#ifdef LOG_REDUCTION
FILE *order_system_call_resolved_initialized[ORDER_THREAD_NUM];
#endif
#endif
...
int vm_init1(JavaVM_Internal * java_vm, JavaVMInitArgs * vm_arguments) {
   ...
   if(!vm_order_record){
#ifdef ORDER_DEBUG
       printf("note , I am here\n");
#endif
       gc_add_thread_map_entry((hythread_t)main_thread);
//       gc_reset_sleep_flags();
   }
#endif
...
//    gc_set_class_class(vm_env->JavaLangClass_Class);
...
#ifdef ORDER
    vm_is_initializing = FALSE;
    vm_initialize_end();
    hythread_vm_initialize_end();
#endif

#ifdef ORDER_DEBUG
    printf("jvm init over\n");
#endif
