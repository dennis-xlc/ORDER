...
#ifdef ORDER //use in replay
     extern bool vm_order_record;
     if(!vm_order_record){
         gc_add_thread_map_entry(native_thread);
//         gc_reset_sleep_flag(native_thread);
     }
#endif
...
#ifdef ORDER
    hythread_t hythread_thread_temp = (hythread_t)vm_thread;
    U_32 p_tid = hythread_thread_temp->p_tid;
    U_32 p_count = hythread_thread_temp->p_count;
#endif
...
#ifdef ORDER
    hythread_thread_temp->p_tid = p_tid;
    hythread_thread_temp->p_count = p_count;
#endif
