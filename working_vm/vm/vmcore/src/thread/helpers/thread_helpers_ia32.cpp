...
#ifdef ORDER
static IDATA rt_jthread_monitor_enter_pure(ManagedObject*  monitor) {
    const unsigned handles_size = (unsigned)(sizeof(ObjectHandlesNew)+sizeof(ManagedObject*)*4);
    ObjectHandlesNew* handels = (ObjectHandlesNew *)STD_ALLOCA(handles_size);
    handels->capacity = 4;
    handels->size = 0;
    ...
    IDATA result = jthread_monitor_enter_pure(monitorJavaObj);
...
char* gen_monitorenter_pure_slow_path_helper(char *ss, const R_Opnd & input_param1) {
    ...
    ss = push(ss, eax_opnd); // push the address of the handle
    ss = call(ss, (char *)rt_jthread_monitor_enter_pure);
    ss = alu(ss, add_opc, esp_opnd, Imm_Opnd(4)); // pop parameters
    return ss;
}
#endif
