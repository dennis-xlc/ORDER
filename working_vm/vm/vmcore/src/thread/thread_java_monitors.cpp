...
{
    ManagedObject *p_obj = *(ManagedObject**)monitor;

//    printf("in monitor enter, (%d, %d), %d\n", p_obj->alloc_tid, p_obj->alloc_count, hythread_self()->thread_id);
...
#ifdef ORDER
    if(!vm_order_record){ // replay mode
        ManagedObject *p_obj = *(ManagedObject**)monitor;
...
check_lock:
        if (!vm_is_initializing && p_obj->alloc_count != -1)
        {
            ...
            while (port_atomic_cas8(&p_obj->access_lock, 1, 0) != 0)
            {
                ...
                p_obj = *(ManagedObject**)monitor;

#ifdef ORDER_DEBUG
                assert(p_obj->alloc_tid % 0x10000 < ORDER_THREAD_NUM && p_obj->alloc_count != 0);
//                printf("after usleep in monitor enter, (%d, %d), %d\n", p_obj->alloc_tid, p_obj->alloc_count, hythread_self()->thread_id);
#endif
            }
...
#ifdef ORDER_DEBUG
                    assert(p_obj->alloc_tid % 0x10000 < ORDER_THREAD_NUM && p_obj->alloc_count != 0);
//                printf("after usleep in monitor enter, tid not match, (%d, %d), current : %d, expected : %d, remaining : %d\n", p_obj->alloc_tid, p_obj->alloc_count, pthread_self(), p_obj->access_tid, p_obj->access_count);
#endif
...
#ifdef ORDER
IDATA VMCALL jthread_monitor_enter_pure(jobject monitor)
{
    ManagedObject *p_obj = *(ManagedObject**)monitor;
...
{
    printf("warning : monitor try enter\n");
...
#ifdef ORDER
extern bool vm_order_record;
FILE *systemCallWaitFile[ORDER_THREAD_NUM];
bool hasPendingNotify[ORDER_THREAD_NUM];
#endif
...
jthread_monitor_timed_wait(jobject monitor, jlong millis, jint nanos)
{
...
#ifdef ORDER
    U_32 status_to_file = TM_ERROR_NONE;
    ...
    else
    {
        ...
        if (systemCallWaitFile[tid] == NULL)
        {
            char name[40];
            sprintf(name, "SYSTEM_CALL_WAIT.%d.log", tid);
    
            systemCallWaitFile[tid] = fopen64(name, "r");
#ifdef ORDER_DEBUG
            if(systemCallWaitFile[tid]== NULL){
                printf("[ORDER]: could NOT open  file %s\n", name);
            }
            if (systemCallWaitFile[tid] != NULL)
                fseek(systemCallWaitFile[tid], 0, SEEK_SET);
#endif
        }
        ...
        {
//#ifdef ORDER_DEBUG
            if (feof(systemCallWaitFile[tid])){
                while(true){
#ifdef ORDER_DEBUG
                    printf("[EOF]: Reach the EOF in jthread_monitor_timed_wait, thread [%d]\n", tid);
#endif
                    usleep(1000);
                    hythread_yield();
                    hythread_safe_point();
...
//#endif
            ...
            fscanf(systemCallWaitFile[tid], "%d", &status_to_file);

//            ManagedObject *p_obj = *(ManagedObject**)monitor;
//            printf("status_to_file : %d, (%d, %d)\n", status_to_file, p_obj->alloc_tid, p_obj->alloc_count);
        }
        else
        {
            status_to_file = 2;
    ...
    else
    {
        ...
        if (millis || nanos)
        {
            if (hasPendingNotify[tid])
            {
                if (status_to_file == TM_ERROR_NONE)
                {
                    hasPendingNotify[tid] = false;
                    status = TM_ERROR_NONE; 
                }
                else if (status_to_file == TM_ERROR_INTERRUPT)
                {
                    hasPendingNotify[tid] = false;
                    status = TM_ERROR_INTERRUPT;
                }
                else if (status_to_file == TM_ERROR_TIMEOUT)
                {
                    status = TM_ERROR_TIMEOUT;
                }
                else if (status_to_file == 2)
                {
                    while(true)
                    {
                        hythread_thin_monitor_wait_interruptable(lockword, millis, nanos);
            ...
            else
            {
                if (status_to_file == TM_ERROR_NONE)
                {
                    status = hythread_thin_monitor_wait(lockword);
#ifdef ORDER_DEBUG
                    assert(status == TM_ERROR_NONE);
#endif
                }
                ...
                else if (status_to_file == TM_ERROR_TIMEOUT)
                {
                    ...
                    if (status == TM_ERROR_NONE || status == TM_ERROR_INTERRUPT)
                        hasPendingNotify[tid] = true;
                }
                else if (status_to_file == 2)
                {
                    while(true)
        ...
        else
        {
            if (status_to_file == TM_ERROR_NONE)
            {
                ...
                {
                    status = hythread_thin_monitor_wait_timed(lockword, millis, nanos);
                }
#ifdef ORDER_DEBUG
                assert(status == TM_ERROR_NONE);
#endif
...
#ifdef ORDER
#ifdef ORDER_DEBUG
    assert(status != TM_ERROR_OUT_OF_MEMORY);
#endif
    if(vm_order_record){
        U_32 tid = hythread_self()->thread_id;
        if (systemCallWaitFile[tid] == NULL){
            ...
            systemCallWaitFile[tid] = fopen64(name, "w+");
#ifdef ORDER_DEBUG
            if(systemCallWaitFile[tid]== NULL){
                printf("[ORDER]: could NOT open  file %s\n", name);
            }
        ...
        {
            status_to_file = TM_ERROR_INTERRUPT;
        }

        if (status == TM_ERROR_TIMEOUT)
        {
            status_to_file = TM_ERROR_TIMEOUT;
        }

        fprintf(systemCallWaitFile[tid], "%d ", status_to_file);
//        fflush(systemCallWaitFile[tid]);
//        printf("status_to_file : %d\n", status_to_file);
    }
    else
    {
    }
