...
#ifdef ORDER
FILE *systemCallMemmove[ORDER_THREAD_NUM];
extern bool vm_order_record;

void order_memmove(void *dst_addr, const void *src_addr, int length)
{
    if(vm_order_record){
        ...
        if (systemCallMemmove[tid] == NULL)
        {
            char name[40];
            sprintf(name, "SYSTEM_CALL_MEMMOVE.%d.log", tid);

            systemCallMemmove[tid] = fopen64(name, "a+");
        }

#ifdef ORDER_DEBUG
        assert(systemCallMemmove[tid] != NULL);
#endif

        fwrite(&length, sizeof(U_32), 1, systemCallMemmove[tid]);

        fwrite(dst_addr, length, 1, systemCallMemmove[tid]);
        ...
        if (systemCallMemmove[tid] == NULL)
        {
            ...
            systemCallMemmove[tid] = fopen64(name, "r");
        }
#ifdef ORDER_DEBUG
        assert(systemCallMemmove[tid] != NULL);
#endif
        ...
        U_32 len_from_log;
        fread(&len_from_log, sizeof(U_32), 1, systemCallMemmove[tid]);
#ifdef ORDER_DEBUG
        assert(len_from_log == length);
#endif
        fread(dst_addr,len_from_log, 1, systemCallMemmove[tid]);
    }
}
#endif
...
#ifdef ORDER
#ifdef ORDER_DEBUG
//            if(hythread_self()->thread_id == 3){
//                printf("[MEMMOVE]: length : %d,  ", length);
//                for(int i=0; i < length; i++){
//                    printf("%c", *(char *)(src_addr + i));
//                }
//                printf("\n");
//                fflush(stdout);
//            }
#endif
            order_memmove(dst_addr, src_addr, (length * sizeof(uint16)));
#else
...
#endif
#ifdef ORDER
        order_memmove(get_vector_element_address_int8(dst, dstOffset),
...
#endif
#ifdef ORDER_N
        order_memmove(get_vector_element_address_bool(dst, dstOffset),
...
#endif
#ifdef ORDER_N
        order_memmove(get_vector_element_address_int16(dst, dstOffset),
...
#endif // VM_STATS
#ifdef ORDER_N
        order_memmove(get_vector_element_address_int32(dst, dstOffset),
...
#endif // VM_STATS
#ifdef ORDER_N
        order_memmove(get_vector_element_address_int64(dst, dstOffset),
...
#endif // VM_STATS
#ifdef ORDER_N
        order_memmove(get_vector_element_address_f32(dst, dstOffset),
...
#endif // VM_STATS
#ifdef ORDER_N
        order_memmove(get_vector_element_address_f64(dst, dstOffset),
