...
{
//printf("in vm instantiate string . \n");
#ifdef ORDER
    U_32 resolve_stat = 0;

    if (vm_is_initializing)
        ;
    else if(vm_order_record){
        ...
        fprintf(order_system_call[tid], "[%d]", 5);
        //int str_len = str->len;
        //fwrite(&str_len, sizeof(str_len), 1, order_system_call[tid]);
        //fprintf(order_system_call[tid], "%d ", str->len);
        fwrite(str->bytes, sizeof(char), str->len, order_system_call[tid]);
//        fprintf(order_system_call[tid], "%s ", str->bytes);

//        if (hythread_self()->thread_id == 13)
//        printf("instantiate string[%d] : %s\n", tid, str->bytes);
...
//        fflush(order_system_call[tid]);
    ...
    else
    {
        ...
        //fscanf(order_system_call[tid], "%d ", &len_from_log);
        //fread(&len_from_log, sizeof(len_from_log), 1, order_system_call[tid]);
        //assert(len_from_log == str->len);
			
        char *temp_buf = (char *)malloc((str->len +1) * sizeof(char));
        fread(temp_buf, sizeof(char), str->len, order_system_call[tid]);
        //fscanf(order_system_call[tid], "%s ", temp_buf);
        temp_buf[str->len] = '\0';
        if (strcmp(temp_buf, str->bytes) != 0)
            assert(0);
#endif
...
//                printf("resolve_stat[%d] : %d\n", tid, (int)resolve_stat);
            ...
            if (resolve_stat == 1)
            {
                while (str->intern.compressed_ref == 0)
                {
                   BEGIN_RAISE_AREA;
