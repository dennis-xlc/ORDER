...
/*
{
    ManagedObject* p_obj = ((ObjectHandle)src)->object;
    VTable* vt = (VTable *) ((UDATA)p_obj->vt_offset + (UDATA)vm_get_vtable_base_address());
    char * class_name = (char *)vt->clss->get_name()->bytes;
    U_32 tid = hythread_self()->thread_id;
    printf("array copy : (%d, %d) -> %d,  %s\n", p_obj->alloc_tid,  p_obj->alloc_count, p_obj->access_count, class_name);
