...
int get_tid()
{

  {
    ...
    int tid_next_temp = 0, pthread_id_temp = 0;
    int current_pthread_id = pthread_self();

    sprintf(name, "THREAD_MAP_WORKING_CLASSLIB.log");

    thread_map = fopen(name, "r");
    if(thread_map == NULL)
        return -1;

    while(1)
    {
        fread(&tid_next_temp, sizeof(int), 1, thread_map);
#ifdef ORDER_DEBUG
        if(feof(thread_map))
        {
            assert(0);
        ...
        fread(&pthread_id_temp, sizeof(int), 1, thread_map);

        if (pthread_id_temp == current_pthread_id)
        {
            fclose(thread_map);
            return tid_next_temp;
        }
    }
  }
...
int notice_fd_list[32];

void add_fd_to_notice_list(I_32 fd)
{
    int i = 0 ;
    for (i = 0 ; i < 32 ; i ++)
    {
        if (notice_fd_list[i] == 0)
        {
            notice_fd_list[i] = fd;
            return;
        }
...
int is_fd_in_notice_list(I_32 fd)
{
    int i = 0 ;
    for (i = 0 ; i < 32 ; i ++)
    {
             ...
             I_32 mode)
{
  ...
  if (strcmp("/dev/urandom", path) == 0 ||
  	strcmp("/dev/random", path) == 0)
  {
#ifdef ORDER_DEBUG
    printf("hyfile open : %s, %d\n", path, get_tid());
#endif
    add_fd_to_notice_list(fd);
  }
...
//ORDER
int vm_mode_determined = FALSE;
int vm_record_mode = FALSE;
FILE* order_input[64];
//ORDER end
             ...
             IDATA nbytes)
{
  ...
  int tid = 0;
//ORDER
  if (is_fd_in_notice_list(fd) == 1)
  {
      tid = get_tid();
      notice = TRUE;
#ifdef ORDER_DEBUG
      printf("read notice data.\n");
#endif

      if (vm_mode_determined == FALSE)
      {
         char name[40];
         sprintf(name, "INPUT.%d.log", tid);

         FILE* tmpFile = fopen(name, "r");

         if (tmpFile == NULL)
         {
             vm_record_mode = TRUE;
             vm_mode_determined = TRUE;
         }
         else
         {
             vm_record_mode = FALSE;
             vm_mode_determined = TRUE;
         }
      }
  }
...
//ORDER
    if (notice == TRUE)
    {
      if (vm_record_mode == TRUE)
      {
//         printf("now in record mode.\n");
         if (order_input[tid] == NULL)
         {
            char name[40];
            sprintf(name, "INPUT.%d.log", tid);

            order_input[tid] = fopen64(name, "a+");
#ifdef ORDER_DEBUG
            assert(order_input[tid]);
#endif
         }

         result = fread(buf, sizeof(char), nbytes, stdin);

         fwrite(&result, sizeof(IDATA), 1, order_input[tid]);
         fwrite(buf, sizeof(char), result, order_input[tid]);
      }
      else
      {
//         printf("now in replay mode.\n");
         if (order_input[tid] == NULL)
         {
            char name[40];
            sprintf(name, "INPUT.%d.log", tid);

            order_input[tid] = fopen64(name, "r");
#ifdef ORDER_DEBUG
            assert(order_input[tid]);
#endif
         }
...
//         result = fread(buf, sizeof(char), nbytes, stdin);
         fread(&result, sizeof(IDATA), 1, order_input[tid]);
         fread(buf, sizeof(char), result, order_input[tid]);
      }
    }
    else
    {
